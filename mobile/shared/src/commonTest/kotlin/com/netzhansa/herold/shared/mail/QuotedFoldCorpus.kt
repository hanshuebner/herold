...
/**
 * The bodies the quoted-history fold is measured against (issue #432).
 *
 * One corpus enumerates the client shapes in combination, in shape
 * order; the other draws the same parts from a seeded sequence, in a
 * different order and with different renderings. Each body carries
 * what it is made of, so a check reads as the contract rather than as
 * a golden string, and every body is measured again with a paragraph
 * ahead of it, after it, and at both ends.
 */
internal object QuotedFoldCorpus {
    ...
    /** Every body, the position permutations included. */
    fun all(): List<FoldCase> {
        val base = enumeratedCorpus() + seededCorpus()
        return base + withTextAround(base)
    }
     ...
     */
    fun withTextAround(corpus: List<FoldCase>): List<FoldCase> = corpus.flatMap { case ->
        val ahead = FoldCase(
    ...
    fun enumeratedCorpus(): List<FoldCase> {
     ...
     */
    fun seededCorpus(): List<FoldCase> {
