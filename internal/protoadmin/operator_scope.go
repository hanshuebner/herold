...
//     (today: PrincipalFlagAdmin + PrincipalFlagSuperAdmin).
//   - {Domains: [...]string}  when caller holds domain:operator or above on one
//     or more domains; the slice is sorted and non-nil even if empty.
...
func ResolveOperatorScope(ctx context.Context, meta store.Metadata, caller store.Principal) OperatorScope {
	lvl, err := authz.Resolve(ctx, meta, caller, authz.Resource{Kind: store.GrantResourceServer})
	if err != nil {
		// Fail closed: a resolution error yields the empty (no-access) scope
		// rather than leaking data or surfacing an internal error.
		return OperatorScope{Domains: emptyDomains}
	}
	if lvl == store.GrantLevelSuperadmin {
		return OperatorScope{SuperAdmin: true}
	}
	domains, err := authz.OperatorDomains(ctx, meta, caller)
	if err != nil {
		return OperatorScope{Domains: emptyDomains}
	}
	if domains == nil {
		// Preserve the nil=unrestricted / non-nil-empty=fail-closed store
		// filter distinction on every non-super-admin path.
