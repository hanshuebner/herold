...
type Metadata interface {
	...
	// -- Sub-account promotion (issue #227, REQ-SUBACCT-09) ------------
	//
	// SubAccountMigration row CRUD. Orchestration (creating the
	// sub-principal, running the reparent/copy sweep, keep-or-purge
	// removal) lives in the free functions store.SeparateIdentity /
	// store.RunSubAccountMigration / store.RemoveSubAccount, which call
	// only these and the other Metadata methods above -- kept out of the
	// interface (and therefore out of both backends' SQL) because they
	// are pure composition, and duplicating composition logic per
	// backend is exactly the drift risk STANDARDS.md guards against.

	// InsertSubAccountMigration creates a new row. ID is generated by
	// the store when m.ID is empty (mirroring
	// CreateIMAPImportAccount). Returns ErrConflict if a row already
	// exists for m.IdentityID or m.SubPrincipalID (both columns carry a
	// unique index): SeparateIdentity's idempotency check must call
	// GetSubAccountMigrationByIdentity first rather than relying on this
	// to race-detect.
	InsertSubAccountMigration(ctx context.Context, m SubAccountMigration) (SubAccountMigration, error)

	// GetSubAccountMigration returns the row by id. Returns ErrNotFound
	// if absent.
	GetSubAccountMigration(ctx context.Context, id string) (SubAccountMigration, error)

	// GetSubAccountMigrationByIdentity returns the row for identityID.
	// Returns ErrNotFound if the Identity has never been separated. The
	// idempotency check SeparateIdentity performs before creating a new
	// sub-principal.
	GetSubAccountMigrationByIdentity(ctx context.Context, identityID string) (SubAccountMigration, error)

	// GetSubAccountMigrationBySubPrincipal returns the row for subID.
	// Returns ErrNotFound if subID was not created by SeparateIdentity
	// (e.g. it is not a sub-principal at all, or predates this
	// bookkeeping). Used by RemoveSubAccount(keep) to resolve which
	// Identity a sub-principal was separated from.
	GetSubAccountMigrationBySubPrincipal(ctx context.Context, subID PrincipalID) (SubAccountMigration, error)

	// ListPendingSubAccountMigrations returns every row with Status
	// pending or running, oldest-created first. Used at boot to resume
	// an interrupted sweep (REQ-IMAP-IMP-107). Returns an empty slice
	// when none.
	ListPendingSubAccountMigrations(ctx context.Context) ([]SubAccountMigration, error)

	// UpdateSubAccountMigrationProgress sets status and lastError, and
	// atomically adds movedDelta/copiedDelta to the row's running
	// counts (relative, not absolute, so concurrent-safe batch-boundary
	// calls never lose a prior batch's progress). When totalIfUnset is
	// non-nil and the row's MessagesTotal is still 0, it is set once to
	// *totalIfUnset (best-effort progress snapshot; never overwritten
	// again). Returns ErrNotFound if id does not exist.
	UpdateSubAccountMigrationProgress(
		ctx context.Context, id string, status SubAccountMigrationStatus,
	...
	// denormalised PrincipalID to newPrincipalID, in place. Companion to
	// RebindJMAPIdentityPrincipal: the two are called together so the
	// account's owning Identity and its own PrincipalID denorm column
	// never disagree. Returns ErrNotFound if accountID does not exist.
	RebindIMAPImportAccountPrincipal(ctx context.Context, accountID string, newPrincipalID PrincipalID) error

	// ReparentMessage atomically moves every membership named in
	// mailboxMoves (source MailboxID -> destination MailboxID) from
	// msgID's current mailbox(es) to the paired destination mailbox(es),
	// and reassigns the message's own owning principal to
	// newPrincipalID. No new message row or blob reference is created --
	// this is a pure relocation, used only when nothing else claims the
	// message (the REQ-IMAP-IMP-106 move case, in both directions: into
	// a sub-account for SeparateIdentity/RunSubAccountMigration, and
	// back out for RemoveSubAccount(keep)). Every key in mailboxMoves
	// must currently be a membership of msgID; the caller (the
	// store.RunSubAccountMigration / store.RemoveSubAccount
	// orchestration) is responsible for passing every membership that
	// must move, since any left out stays behind under the old
	// principal, splitting the message across two accounts.
	//
	// Idempotent: because every mutation lands in one transaction, a
	// msgID whose principal already equals newPrincipalID is a no-op
	// success (a prior, possibly-interrupted run already completed it;
	// "completed" and "not started" are the only two reachable states).
	// Returns ErrNotFound if msgID does not exist.
	ReparentMessage(ctx context.Context, msgID MessageID, newPrincipalID PrincipalID, mailboxMoves map[MailboxID]MailboxID) error

	// CopyImportedMessageToSubAccount atomically inserts req.NewMessage
	// (a fresh message row under the sub-principal, referencing the same
	// blob as req.SourceMessageID -- blob refcount incremented) targeting
	// req.Targets, and records the resulting MessageID as the durable
	// "copied" marker on every imapimport_message_state row for
	// (req.AccountID, req.SourceMessageID), in the same transaction. The
	// source message and its existing memberships are untouched (the
	// REQ-IMAP-IMP-103/106 dedup-safe copy case: the source is also
	// claimed by another channel, so it must survive under the parent).
	//
	// Idempotent: if the marker is already set (a prior, possibly-
	// interrupted run already completed the copy), returns the existing
	// target MessageID with AlreadyDone = true and creates nothing.
	CopyImportedMessageToSubAccount(ctx context.Context, req CopyImportedMessageRequest) (CopyImportedMessageResult, error)
