...
var intentionalExclusions = map[string]string{
	...
	// blob_part_index is a pure cache keyed on content-addressed blob_hash;
	// the payload is fully derivable from the immutable blob bytes and can
	// always be regenerated by the part-index worker after a restore.
	// Backing it up would waste space without improving recovery fidelity.
	"blob_part_index": "regeneratable cache derived from immutable content-addressed blobs",
}
