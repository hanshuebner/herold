...
func (h *memChangesHandler) Execute(ctx context.Context, args json.RawMessage) (any, *protojmap.MethodError) {
	...
	currentSeq, err := h.h.store.Meta().GetMaxChangeSeqForKind(ctx, pid, store.EntityKindMembership)
	if err != nil {
		return nil, serverFail(err)
	}
	...
	created, updated, destroyed, cutoff, hasMore, ferr := protojmap.WalkChangesBySeq(
		ctx, h.h.store.Meta(), pid, store.EntityKindMembership, store.ChangeSeq(since), maxChanges)
	if ferr != nil {
