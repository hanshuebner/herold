...
// mapping methods (epic #188, REQ-AC-60..70). Schema commentary lives in
// migrations/0083_oidc_claim_mapping.sql; the grant reconciliation this
// mapping feeds is internal/authz.ReconcileIdP.
...
func (m *metadata) DeleteClaimAllowlistEntry(ctx context.Context, providerName, claim string) error {
	return m.runTx(ctx, func(tx *sql.Tx) error {
		res, err := tx.ExecContext(ctx, `
			DELETE FROM oidc_claim_allowlist WHERE provider_name = ? AND claim = ?`,
			providerName, claim)
		if err != nil {
			return mapErr(err)
		}
...
func (m *metadata) ListClaimAllowlist(ctx context.Context, providerName string) ([]string, error) {
	rows, err := m.s.db.QueryContext(ctx, `
		SELECT claim FROM oidc_claim_allowlist WHERE provider_name = ? ORDER BY claim`,
		providerName)
	if err != nil {
...
func (m *metadata) InsertClaimMappingRule(ctx context.Context, rule store.ClaimMappingRule) (store.ClaimMappingRule, error) {
	...
	if err != nil {
		return store.ClaimMappingRule{}, err
	}
...
func (m *metadata) DeleteClaimMappingRule(ctx context.Context, id store.ClaimMappingRuleID) error {
	return m.runTx(ctx, func(tx *sql.Tx) error {
		res, err := tx.ExecContext(ctx,
			`DELETE FROM oidc_claim_mapping_rules WHERE id = ?`, int64(id))
		if err != nil {
			return mapErr(err)
		}
		n, _ := res.RowsAffected()
...
// semantics; this is the SQLite transaction that realises it.
	...
	err = m.runTx(ctx, func(tx *sql.Tx) error {
		rows, qerr := tx.QueryContext(ctx, `
			SELECT id, `+grantColumns+`
			  FROM grants
			 WHERE subject_kind = 'principal' AND subject_id = ? AND provenance = ?`,
		...
		for k, level := range desiredByKey {
			...
			if cur.Level != level {
				if _, uerr := tx.ExecContext(ctx, `
					UPDATE grants SET level = ?, last_asserted_at_us = ?
					 WHERE id = ?`, string(level), usMicros(asOf), int64(cur.ID)); uerr != nil {
					return mapErr(uerr)
				}
			...
			if _, uerr := tx.ExecContext(ctx, `
				UPDATE grants SET last_asserted_at_us = ? WHERE id = ?`,
		...
		for k, cur := range currentByKey {
			...
			if _, derr := tx.ExecContext(ctx, `DELETE FROM grants WHERE id = ?`, int64(cur.ID)); derr != nil {
				return mapErr(derr)
			}
			removed = append(removed, cur)
		}
...
func (m *metadata) SweepStaleIdPGrants(ctx context.Context, olderThan time.Time) ([]store.Grant, error) {
	...
	err := m.runTx(ctx, func(tx *sql.Tx) error {
		rows, qerr := tx.QueryContext(ctx, `
			SELECT id, `+grantColumns+`
			 ...
			 WHERE provenance LIKE 'idp:%' AND last_asserted_at_us IS NOT NULL
			   AND last_asserted_at_us < ?`, usMicros(olderThan))
		if qerr != nil {
			return mapErr(qerr)
		}
		...
		for _, g := range stale {
			if _, derr := tx.ExecContext(ctx, `DELETE FROM grants WHERE id = ?`, int64(g.ID)); derr != nil {
				return mapErr(derr)
			}
