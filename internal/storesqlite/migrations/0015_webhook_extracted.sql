...
-- text_required drop policy). Mirrors storepg 0015. Forward-only.
--
...
--
-- The defaults match the existing behaviour: a row created before this
-- migration ran has target_kind=0 and body_mode=0, both of which mean
-- "use the legacy column".  Code paths that read either field MUST
-- prefer the new column when non-zero and fall back otherwise.
...
ALTER TABLE webhooks ADD COLUMN extracted_text_max_bytes INTEGER NOT NULL DEFAULT 0;
ALTER TABLE webhooks ADD COLUMN text_required            INTEGER NOT NULL DEFAULT 0;
