...
func (m *metadata) ListQueueItems(ctx context.Context, filter store.QueueFilter) ([]store.QueueItem, error) {
	...
	if len(filter.SenderDomains) > 0 {
		...
		where = append(where,
			"lower(substr(mail_from, instr(mail_from, '@') + 1)) IN ("+
				strings.Join(placeholders, ",")+")")
	}
	if filter.MailFromContains != "" {
		where = append(where, "lower(mail_from) LIKE lower('%'||?||'%')")
		args = append(args, filter.MailFromContains)
	}
...
func (m *metadata) InsertEmailSubmission(ctx context.Context, row store.EmailSubmissionRow) error {
	...
	return m.runTx(ctx, func(tx *sql.Tx) error {
		...
		heldVal := int64(0)
		if row.HeldForReauth {
			heldVal = 1
		}
		var holdDeadlineArg any
...
func (m *metadata) ListEmailSubmissionsHeldForReauth(ctx context.Context, identityID string) ([]store.EmailSubmissionRow, error) {
	rows, err := m.s.db.QueryContext(ctx, `
		SELECT `+emailSubmissionSelectColumns+`
		 ...
		 WHERE held_for_reauth = 1 AND identity_id = ?
	...
	return m.runTx(ctx, func(tx *sql.Tx) error {
		res, err := tx.ExecContext(ctx, `
			UPDATE jmap_email_submissions
			   SET held_for_reauth = ?, undo_status = ?, properties = ?
			 WHERE id = ?`,
			heldVal, undoStatus, properties, id)
		if err != nil {
			return mapErr(err)
		}
		n, err := res.RowsAffected()
...
// attachJMAPIdentityAliases populates id.Aliases from
...
func (m *metadata) attachJMAPIdentityAliases(ctx context.Context, id *store.JMAPIdentity) error {
	rows, err := m.s.db.QueryContext(ctx,
		`SELECT address FROM jmap_identity_aliases WHERE identity_id = ? ORDER BY position ASC`,
		id.ID)
	if err != nil {
		return mapErr(err)
	}
...
// validateJMAPIdentityAliases checks candidate aliases against email
// and every other identity owned by principalID (REQ-IDENT-01, re
// #387), returning the trimmed alias list to write on success:
//   - each alias must be ValidateEmailAddressSyntax-valid;
//   - no two aliases may be the same address (case-insensitive);
//   - no alias may equal email itself;
//   - no alias may equal another identity's primary address;
//   - no alias may equal another identity's existing alias;
//   - email itself may not already be registered as another
//     identity's existing alias (the reverse of the same invariant).
//
// selfID excludes the identity being written from the "another
// identity" checks; passing the id of a row that does not exist yet
// (the Insert path) is harmless since nothing can reference it. Runs
// inside tx so the checks are consistent with the write that follows.
func validateJMAPIdentityAliases(ctx context.Context, tx *sql.Tx, principalID store.PrincipalID, selfID, email string, aliases []string) ([]string, error) {
	...
	emailRows, err := tx.QueryContext(ctx,
		`SELECT id, email FROM jmap_identities WHERE principal_id = ? AND id != ?`,
	...
	aliasRows, err := tx.QueryContext(ctx,
		`SELECT identity_id, address FROM jmap_identity_aliases WHERE principal_id = ? AND identity_id != ?`,
		int64(principalID), selfID)
	if err != nil {
		return nil, mapErr(err)
	}
...
// insertJMAPIdentityAliases writes aliases for identityID in order
// (position 0..n-1). Callers run this after validateJMAPIdentityAliases
// inside the same transaction.
func insertJMAPIdentityAliases(ctx context.Context, tx *sql.Tx, principalID store.PrincipalID, identityID string, aliases []string) error {
	for i, a := range aliases {
		if _, err := tx.ExecContext(ctx,
			`INSERT INTO jmap_identity_aliases (identity_id, principal_id, address, position) VALUES (?, ?, ?, ?)`,
...
func (m *metadata) InsertJMAPIdentity(ctx context.Context, row store.JMAPIdentity) error {
	...
	return m.runTx(ctx, func(tx *sql.Tx) error {
		...
		aliases, err := validateJMAPIdentityAliases(ctx, tx, row.PrincipalID, row.ID, row.Email, row.Aliases)
		if err != nil {
			return err
		}
		...
		_, err = tx.ExecContext(ctx, `
			...
			boolToInt(row.IsDefault))
		if err != nil {
			return mapErr(err)
		}
		return insertJMAPIdentityAliases(ctx, tx, row.PrincipalID, row.ID, aliases)
	})
}
...
func (m *metadata) UpdateJMAPIdentity(ctx context.Context, row store.JMAPIdentity) error {
	...
	return m.runTx(ctx, func(tx *sql.Tx) error {
		...
		var email string
		err := tx.QueryRowContext(ctx,
			`SELECT principal_id, email FROM jmap_identities WHERE id = ?`, row.ID).
		...
		aliases, err := validateJMAPIdentityAliases(ctx, tx, store.PrincipalID(principalID), row.ID, email, row.Aliases)
		if err != nil {
			return err
		}
		replyTo := row.ReplyToJSON
		...
		if _, err := tx.ExecContext(ctx,
			`DELETE FROM jmap_identity_aliases WHERE identity_id = ?`, row.ID); err != nil {
			return mapErr(err)
		}
		return insertJMAPIdentityAliases(ctx, tx, store.PrincipalID(principalID), row.ID, aliases)
	})
}
...
func (m *metadata) PrincipalOwnsIdentityAlias(ctx context.Context, principalID store.PrincipalID, addr string) (string, bool, error) {
	...
	err := m.s.db.QueryRowContext(ctx,
		`SELECT identity_id FROM jmap_identity_aliases
		  WHERE principal_id = ? AND lower(address) = lower(?)
		  LIMIT 1`,
...
func (m *metadata) SetDefaultJMAPIdentity(ctx context.Context, principalID store.PrincipalID, identityID string) error {
	return m.runTx(ctx, func(tx *sql.Tx) error {
		if identityID != "default" {
			var ownerID int64
			err := tx.QueryRowContext(ctx,
				`SELECT principal_id FROM jmap_identities WHERE id = ?`,
		...
		if _, err := tx.ExecContext(ctx,
			`UPDATE jmap_identities SET is_default = 0 WHERE principal_id = ?`,
		...
		res, err := tx.ExecContext(ctx,
			`UPDATE jmap_identities SET is_default = 1 WHERE id = ?`,
			identityID)
		if err != nil {
			return mapErr(err)
		}
...
// exists for every entry in cats (issue #406, ADR-0004): "a category is a
// label" only holds once the categoriser's derived set actually has a
// backing Mailbox to carry a disposition and priority. An existing
// mailbox of that name is adopted untouched — its disposition and
// priority (whether left at the default or since changed by the user)
// are never rewritten by a recompute. A newly created label defaults to
// MailboxDispositionPinned with the next dense priority rank, appended
// after the principal's existing ranked labels. A category absent from a
// later cats slice is not touched here — its label and disposition stay
// exactly as they are.
func (m *metadata) SetDerivedCategories(ctx context.Context, pid store.PrincipalID, categories []string, expectedEpoch int64) (bool, error) {
	...
	err := m.runTx(ctx, func(tx *sql.Tx) error {
		...
		return ensureCategoryLabelMailboxes(ctx, tx, m.s.clock.Now().UTC(), m.s.randReader, pid, cats)
	})
	return updated, err
}

// ensureCategoryLabelMailboxes ensures a label mailbox exists for every
...
func ensureCategoryLabelMailboxes(ctx context.Context, tx *sql.Tx, now time.Time, randReader io.Reader, pid store.PrincipalID, cats []string) error {
	if len(cats) == 0 {
		return nil
	}
	var nextRank int64
	if err := tx.QueryRowContext(ctx,
		`SELECT COUNT(*) FROM mailboxes WHERE principal_id = ? AND priority IS NOT NULL`,
	...
	for _, name := range cats {
		...
		var exists int64
		if err := tx.QueryRowContext(ctx,
			`SELECT COUNT(*) FROM mailboxes WHERE principal_id = ? AND name = ?`,
		...
		res, err := tx.ExecContext(ctx, `
			...
			VALUES (?, 0, ?, 0, ?, 1, 0, ?, ?, NULL, 0, ?, ?)`,
			int64(pid), name, int64(uidValidity), usMicros(now), usMicros(now),
			string(store.MailboxDispositionPinned), nextRank)
		if err != nil {
			return fmt.Errorf("storesqlite: category label mailbox %q: %w", name, mapErr(err))
		}
...
// EnsureCategoryLabelMailboxes runs ensureCategoryLabelMailboxes in its own
// transaction, independent of any derived_categories_json write (issue
...
// mailbox -- is checked with a plain read on m.s.db first, outside any
// transaction. Only when that read finds a missing name does this open
// the write transaction (runTx, the global writer mutex, BEGIN
// IMMEDIATE), so the common case of a classifier repeatedly returning
// the same category set costs one read per message, not one write
// transaction.
func (m *metadata) EnsureCategoryLabelMailboxes(ctx context.Context, pid store.PrincipalID, categories []string) error {
	...
	complete, err := categoryLabelMailboxesComplete(ctx, m.s.db, pid, cats)
	if err != nil {
		return err
	}
	if complete {
...
// categoryLabelMailboxesComplete reports whether every name in cats
// already has a backing mailbox for pid, via a plain read on q (either
// *sql.DB or *sql.Tx). It is the read-only fast path EnsureCategoryLabel
// Mailboxes uses to skip opening a write transaction when there is
// nothing to create.
func categoryLabelMailboxesComplete(ctx context.Context, q interface {
	...
	rows, err := q.QueryContext(ctx,
		`SELECT name FROM mailboxes WHERE principal_id = ? AND name IN (`+strings.Join(placeholders, ",")+`)`,
		args...)
	if err != nil {
		return false, mapErr(err)
	}
	...
	err = m.runTx(ctx, func(tx *sql.Tx) error {
		_, err := tx.ExecContext(ctx, `
			...
			ON CONFLICT(message_id) DO UPDATE SET
			  ...
			  spam_reason              = COALESCE(excluded.spam_reason, spam_reason),
			  ...
			  category_classified_at_us = COALESCE(excluded.category_classified_at_us, category_classified_at_us)`,
...
func (m *metadata) ensureDerivedCategoryFromAssignment(ctx context.Context, pid store.PrincipalID, assigned string) {
	...
	if err != nil {
		if m.s.logger != nil {
			m.s.logger.Warn("storesqlite: category label mailbox heal: load config failed",
	...
	if stringSliceEqualSQ(names, cfg.DerivedCategories) {
		if err := m.EnsureCategoryLabelMailboxes(ctx, pid, names); err != nil && m.s.logger != nil {
			m.s.logger.Warn("storesqlite: category label mailbox heal failed",
				"principal_id", int64(pid), "err", err.Error())
		}
		return
	}
	if _, err := m.SetDerivedCategories(ctx, pid, names, cfg.DerivedCategoriesEpoch); err != nil && m.s.logger != nil {
		m.s.logger.Warn("storesqlite: category label mailbox heal via SetDerivedCategories failed",
			"principal_id", int64(pid), "err", err.Error())
	}
}

// stringSliceEqualSQ reports whether a and b are element-wise equal.
func stringSliceEqualSQ(a, b []string) bool {
	if len(a) != len(b) {
		return false
	}
	for i := range a {
...
func (m *metadata) BatchGetLLMClassifications(ctx context.Context, msgIDs []store.MessageID) (map[store.MessageID]store.LLMClassificationRecord, error) {
	...
	for rows.Next() {
		...
		if spamConfidence.Valid {
			v := spamConfidence.Float64
			rec.SpamConfidence = &v
		}
		if spamReason.Valid {
			v := spamReason.String
			rec.SpamReason = &v
		}
		if spamPromptApplied.Valid {
			v := spamPromptApplied.String
			rec.SpamPromptApplied = &v
		}
		if spamModel.Valid {
			v := spamModel.String
			rec.SpamModel = &v
		...
		if categoryAssigned.Valid {
			...
			rec.CategoryAssigned = &v
		}
		if categoryPromptApplied.Valid {
			v := categoryPromptApplied.String
			rec.CategoryPromptApplied = &v
		}
		if categoryModel.Valid {
			v := categoryModel.String
			rec.CategoryModel = &v
		}
		if categoryClassifiedAtUs.Valid {
			t := fromMicros(categoryClassifiedAtUs.Int64)
			rec.CategoryClassifiedAt = &t
		}
		out[store.MessageID(msgID)] = rec
	}
	return out, mapErr(rows.Err())
}

// optTimeToUs converts an optional *time.Time to a nullable int64 (Unix
// micros) for SQL binding. Nil input → nil (SQL NULL).
func optTimeToUs(t *time.Time) any {
	if t == nil {
		return nil
	}
	return usMicros(*t)
