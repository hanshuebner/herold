...
func (m *metadata) InsertPrincipal(ctx context.Context, p store.Principal) (store.Principal, error) {
	...
	err := m.runTx(ctx, func(tx *sql.Tx) error {
		...
		res, err := tx.ExecContext(ctx, `
			...
			nullable(telemetry), nullablePrincipalID(p.ParentPrincipalID))
		if err != nil {
...
func (m *metadata) selectPrincipal(ctx context.Context, where string, args ...any) (store.Principal, error) {
	...
	if parentID.Valid {
		p.ParentPrincipalID = store.PrincipalID(parentID.Int64)
	}
...
func quotaPrincipalID(ctx context.Context, tx *sql.Tx, pid int64) (int64, error) {
	var parent sql.NullInt64
	if err := tx.QueryRowContext(ctx,
		`SELECT parent_principal_id FROM principals WHERE id = ?`, pid).Scan(&parent); err != nil {
		return 0, mapErr(err)
	}
	if parent.Valid {
...
func (m *metadata) InsertSubPrincipal(ctx context.Context, parentID store.PrincipalID, p store.Principal) (store.Principal, error) {
	...
	var id int64
	err := m.runTx(ctx, func(tx *sql.Tx) error {
		var parentKind int64
		var parentOfParent sql.NullInt64
		if err := tx.QueryRowContext(ctx,
			`SELECT kind, parent_principal_id FROM principals WHERE id = ?`,
			int64(parentID)).Scan(&parentKind, &parentOfParent); err != nil {
			return mapErr(err)
		}
		if store.PrincipalKind(parentKind) != store.PrincipalKindUser {
			return fmt.Errorf(
				"storesqlite: sub-principal parent must be an individual principal: %w", store.ErrInvalidArgument)
		}
		if parentOfParent.Valid {
			return fmt.Errorf(
				"storesqlite: sub-principals cannot nest: %w", store.ErrInvalidArgument)
		}
		res, err := tx.ExecContext(ctx, `
			INSERT INTO principals (kind, canonical_email, display_name, password_hash,
			  totp_secret, quota_bytes, flags, used_bytes, created_at_us, updated_at_us,
			  parent_principal_id)
			VALUES (?, ?, ?, '', NULL, 0, ?, 0, ?, ?, ?)`,
			int64(store.PrincipalKindSubAccount), strings.ToLower(p.CanonicalEmail), p.DisplayName,
...
func (m *metadata) ListSubPrincipals(ctx context.Context, parentID store.PrincipalID) ([]store.Principal, error) {
	rows, err := m.s.db.QueryContext(ctx, `
		SELECT id, kind, canonical_email, display_name, password_hash, totp_secret,
		  ...
		  FROM principals WHERE parent_principal_id = ? ORDER BY id ASC`,
		int64(parentID))
	if err != nil {
...
func (m *metadata) InsertAlias(ctx context.Context, a store.Alias) (store.Alias, error) {
	...
	err := m.runTx(ctx, func(tx *sql.Tx) error {
		res, err := tx.ExecContext(ctx, `
			...
			nullable(targetPrincipal), nullStringPtr(targetAddress), nullable(expiresUs), usMicros(now))
		if err != nil {
...
func (m *metadata) ResolveAlias(ctx context.Context, localPart, domain string) (store.PrincipalID, error) {
	...
	if err == nil {
		return store.PrincipalID(target.Int64), nil
	}
	if !errors.Is(err, sql.ErrNoRows) {
		return 0, mapErr(err)
	}
...
func (m *metadata) ResolveAliasExternalTarget(ctx context.Context, localPart, domain string) (string, error) {
	...
	err := m.s.db.QueryRowContext(ctx, `
		SELECT target_address FROM aliases
		 WHERE local_part = ? AND domain = ? AND target_address IS NOT NULL
		   AND (expires_at_us IS NULL OR expires_at_us > ?)`,
		lp, dom, now).Scan(&target)
	if err != nil {
		return "", mapErr(err)
...
func (m *metadata) ListAliases(ctx context.Context, domain string) ([]store.Alias, error) {
	...
	for rows.Next() {
		...
		a.ID = store.AliasID(id)
		if target.Valid {
			a.TargetPrincipal = store.PrincipalID(target.Int64)
		}
		if targetAddr.Valid {
			a.TargetAddress = targetAddr.String
		}
	...
	return m.runTx(ctx, func(tx *sql.Tx) error {
		...
		for _, bi := range soleBlobs {
			...
			if _, err := tx.ExecContext(ctx, `
				...
				bi.size, usMicros(m.s.clock.Now()), qpid); err != nil {
				return mapErr(err)
			}
		}
...
func scanMessageRow(row rowLike) (store.Message, error) {
	...
	var retryableFailedImageCount int64
	var failedImageReason string
	var ingestSource string
	var snoozeWokeAtUs sql.NullInt64
	var snoozeWokeForUs sql.NullInt64
	...
	msg.RetryableFailedImageCount = int(retryableFailedImageCount)
	msg.FailedImageReason = failedImageReason
	msg.IngestSource = store.MessageIngestSource(ingestSource)
	if snoozeWokeAtUs.Valid {
		t := fromMicros(snoozeWokeAtUs.Int64)
		msg.SnoozeWokeAt = &t
	}
	if snoozeWokeForUs.Valid {
		t := fromMicros(snoozeWokeForUs.Int64)
		msg.SnoozeWokeFor = &t
	}
	return msg, nil
}
...
func (m *metadata) insertMessageTx(
	...
	err := func() error {
		...
		// Quota check. A sub-account's mail counts against its parent's
		// quota (REQ-SUBACCT-05): resolve the quota-owning principal
		// before reading quota_bytes/used_bytes.
		qpid, err := quotaPrincipalID(ctx, tx, pid)
		if err != nil {
			return err
		}
		var quota, used int64
		if err := tx.QueryRowContext(ctx, `SELECT quota_bytes, used_bytes FROM principals WHERE id = ?`, qpid).Scan(&quota, &used); err != nil {
			return mapErr(err)
		}
		...
		if msg.ThreadID == 0 && !skipThreading && msg.Envelope.MessageID != "" {
			...
			sameErr := tx.QueryRowContext(ctx, `
				SELECT id, thread_id
				  FROM messages
				 WHERE principal_id = ?
				   AND env_message_id = ?
			...
			} else if sameErr != sql.ErrNoRows {
				return fmt.Errorf("storesqlite: same-message-id thread lookup: %w", sameErr)
			}
		}
...
func mergeLateAncestorThreads(
	ctx context.Context, tx *sql.Tx, pid int64,
	...
	// (REQ-STORE-40 reverse index, migration 0115). A single seek on
	// idx_message_references_principal_referenced(principal_id,
	// referenced_message_id) followed by a primary-key join, versus the
	// LOWER(...) LIKE '%...%' scan of every message row for the principal
	// this replaced (EXPLAIN QUERY PLAN showed a full linear evaluation
	// after the principal_id index seek).
	...
	compQ := `
		SELECT id, env_message_id, env_in_reply_to, env_references, env_subject, thread_id
		 ...
		 ORDER BY internal_date_us ASC, id ASC`
	compRows, err := tx.QueryContext(ctx, compQ, args...)
	if err != nil {
		return mapErr(err)
	}
	...
	for _, u := range updates {
		if _, err := tx.ExecContext(ctx, `UPDATE messages SET thread_id = ? WHERE id = ?`,
		...
		mbRows, err := tx.QueryContext(ctx,
			`SELECT mailbox_id FROM message_mailboxes WHERE message_id = ?`, u.ID)
		if err != nil {
			return mapErr(err)
		}
...
func writeMessageReferences(ctx context.Context, tx *sql.Tx, pid int64, msgID store.MessageID, refs []string) error {
	if len(refs) == 0 {
		return nil
	}
	...
	defer stmt.Close()
	for _, ref := range refs {
		if _, err := stmt.ExecContext(ctx, pid, int64(msgID), ref); err != nil {
			return mapErr(err)
		}
	}
	return nil
...
func (m *metadata) ReplaceMessageBody(
	...
	return m.runTx(ctx, func(tx *sql.Tx) error {
		...
		if delta := size - oldSize; delta > 0 {
			var quota, used int64
			if err := tx.QueryRowContext(ctx,
				`SELECT quota_bytes, used_bytes FROM principals WHERE id = ?`, qpid).Scan(&quota, &used); err != nil {
				return mapErr(err)
			}
		...
		// retryable_failed_image_count / failed_image_reason (issue
		// #271) are reset alongside them for the same reason.
		if _, err := tx.ExecContext(ctx, `
			UPDATE messages
			   SET blob_hash = ?, blob_size = ?, size = ?,
			       ...
			       retryable_failed_image_count = 0, failed_image_reason = ''
			 WHERE id = ?`,
		...
		if _, err := tx.ExecContext(ctx,
			`DELETE FROM message_references WHERE message_id = ?`, int64(id)); err != nil {
			return mapErr(err)
		}
		...
		// emit one EntityKindEmail/ChangeOpUpdated entry per mailbox.
		// The state-change row is tagged cause = 'background'
		// (REQ-EXTIMG-BG-INTERNAL-15): IMAP CONDSTORE / FETCH still
		// observe the rewrite via the modseq advance and via
		// ReadChangeFeedAll, but JMAP Email/changes and the EventSource
		// push loop suppress it so the SPA does not fan out one
		// /changes round-trip per worker rewrite. ReplaceMessageBody is
		// the only v1 background writer; live-mail callers route
		// through different paths and continue to write cause = 'user'.
		rows, err := tx.QueryContext(ctx,
...
func (m *metadata) SetMessageFailedImages(ctx context.Context, msgID store.MessageID, count int, stateJSON string, retryableCount int, reason string) error {
	res, err := m.s.db.ExecContext(ctx,
		`UPDATE messages SET failed_image_count = ?, failed_image_state = ?,
		   retryable_failed_image_count = ?, failed_image_reason = ? WHERE id = ?`,
		count, stateJSON, retryableCount, reason, int64(msgID))
	if err != nil {
		return mapErr(err)
	}
	n, err := res.RowsAffected()
...
// SQLite's row-value comparison so the query plan stays obvious to
// future readers and the same SQL shape transports cleanly across
// both backends. Zero sentinels for the cursor map to math.MaxInt64
// so the first call (beforeReceivedAtUs=0, beforeMessageID=0) returns
...
func (m *metadata) ListMessagesWithInternalizePendingByReceivedAt(
	...
	for rows.Next() {
		...
		if err := rows.Scan(&id, &receivedAt); err != nil {
			return nil, fmt.Errorf("storesqlite: list pending by received_at: scan: %w", err)
		}
...
func (m *metadata) UpdateMessageFlags(
	...
	err := m.runTx(ctx, func(tx *sql.Tx) error {
		...
		if flagAdd&store.MessageFlagSeen != 0 {
			...
			if _, err := tx.ExecContext(ctx, `
				UPDATE messages SET snooze_woke_at_us = NULL, snooze_woke_for_us = NULL WHERE id = ?`,
				int64(id)); err != nil {
				return mapErr(err)
			}
		}
...
func (m *metadata) MoveMessage(ctx context.Context, msgID store.MessageID, fromMailboxID, targetMailboxID store.MailboxID) error {
	...
	return m.runTx(ctx, func(tx *sql.Tx) error {
		...
		var tgtAttrs int64
		if err := tx.QueryRowContext(ctx, `SELECT principal_id, uidnext, highest_modseq, attributes FROM mailboxes WHERE id = ?`,
			int64(targetMailboxID)).Scan(&pid, &tgtUIDNext, &tgtHighest, &tgtAttrs); err != nil {
			if errors.Is(err, sql.ErrNoRows) {
				return store.ErrNotFound
			}
			return mapErr(err)
		...
		if isToTrash {
			if err := snapshotPretrashMailboxes(ctx, tx, int64(msgID), int64(targetMailboxID)); err != nil {
				return err
			}
		}
		...
		var srcAttrs int64
		if err := tx.QueryRowContext(ctx, `SELECT attributes FROM mailboxes WHERE id = ?`,
			int64(fromMailboxID)).Scan(&srcAttrs); err != nil && !errors.Is(err, sql.ErrNoRows) {
			return mapErr(err)
		}
		isFromTrash := store.MailboxAttributes(srcAttrs)&store.MailboxAttrTrash != 0
		if isFromTrash {
			if err := replayPretrashMailboxes(ctx, tx, int64(msgID), now); err != nil {
				return err
			}
		}
...
func (m *metadata) RemoveMessageFromMailbox(ctx context.Context, msgID store.MessageID, mailboxID store.MailboxID) error {
	...
	return m.runTx(ctx, func(tx *sql.Tx) error {
		...
		var attrs int64
		if err := tx.QueryRowContext(ctx, `SELECT attributes FROM mailboxes WHERE id = ?`,
			int64(mailboxID)).Scan(&attrs); err != nil && !errors.Is(err, sql.ErrNoRows) {
			return mapErr(err)
		}
...
// snapshotPretrashMailboxes records the current non-Trash mailbox memberships
...
func snapshotPretrashMailboxes(ctx context.Context, tx *sql.Tx, msgID, trashMailboxID int64) error {
	// Clear any prior snapshot.
	if _, err := tx.ExecContext(ctx,
		`DELETE FROM email_pretrash_mailboxes WHERE email_id = ?`, msgID); err != nil {
		return err
	}
...
// replayPretrashMailboxes restores the pre-trash mailbox memberships for msgID
// that were recorded by snapshotPretrashMailboxes. Each mailbox is re-added if
// the mailbox still exists and the message is not already a member. The snapshot
// is deleted after replay.
func replayPretrashMailboxes(ctx context.Context, tx *sql.Tx, msgID int64, now time.Time) error {
	rows, err := tx.QueryContext(ctx,
		`SELECT mailbox_id FROM email_pretrash_mailboxes WHERE email_id = ?`, msgID)
	if err != nil {
		return err
	}
	var mailboxIDs []int64
	...
	for _, mbID := range mailboxIDs {
		// Check mailbox still exists and get uidnext/highest_modseq.
		var uidNext, highest int64
		err := tx.QueryRowContext(ctx,
			`SELECT uidnext, highest_modseq FROM mailboxes WHERE id = ?`, mbID).
		...
		// Check message is not already a member.
		var cnt int64
		if err := tx.QueryRowContext(ctx,
			`SELECT COUNT(*) FROM message_mailboxes WHERE message_id = ? AND mailbox_id = ?`,
...
func (m *metadata) readChangeFeed(
	...
	// The default ReadChangeFeed path filters cause = 'user' so JMAP
	// /changes / EventSource consumers do not see the extimg
	// internalize-worker's background body rewrites
	// (REQ-EXTIMG-BG-INTERNAL-11). The partial index
	// idx_state_changes_principal_kind_seq_user from migration 0044
	// keeps that filter cheap. ReadChangeFeedAll skips the predicate
	// for IMAP IDLE / FTS / seen-address consumers that need every
	// cause.
	var rows *sql.Rows
	var err error
	if userOnly {
		rows, err = m.s.db.QueryContext(ctx, `
			SELECT seq, principal_id, entity_kind, entity_id, parent_entity_id, op, cause, produced_at_us
			  FROM state_changes
			 WHERE principal_id = ? AND seq > ? AND cause = 'user'
...
func (m *metadata) GetMaxChangeSeqForKind(
	...
	// User-cause filter (REQ-EXTIMG-BG-INTERNAL-10): a background
	// rewrite must not advance the JMAP-visible state for the affected
	// kind. The partial index idx_state_changes_principal_kind_seq_user
	// from migration 0044 makes the MAX cheap.
	var seq int64
	err := m.s.db.QueryRowContext(ctx,
		`SELECT COALESCE(MAX(seq), 0) FROM state_changes
		 WHERE principal_id = ? AND entity_kind = ? AND cause = 'user'`,
...
func appendStateChangeSeqWithCause(
	...
	_, err = tx.ExecContext(ctx, `
		INSERT INTO state_changes (principal_id, seq, entity_kind, entity_id,
		  parent_entity_id, op, cause, produced_at_us) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
		int64(principalID), next, string(kind), int64(entityID),
		int64(parentEntityID), int(op), string(cause), usMicros(now))
	if err != nil {
		return 0, mapErr(err)
	}
...
func (m *metadata) ListPrincipals(ctx context.Context, after store.PrincipalID, limit int) ([]store.Principal, error) {
	...
	rows, err := m.s.db.QueryContext(ctx, `
		SELECT id, kind, canonical_email, display_name, password_hash, totp_secret,
		       quota_bytes, flags, created_at_us, updated_at_us
		  FROM principals WHERE id > ? AND kind != ? ORDER BY id ASC LIMIT ?`,
		int64(after), int64(store.PrincipalKindSubAccount), limit)
	if err != nil {
		return nil, mapErr(err)
	}
	defer rows.Close()
...
func (m *metadata) SearchPrincipalsByText(ctx context.Context, prefix string, limit int) ([]store.Principal, error) {
	...
	rows, err := m.s.db.QueryContext(ctx, `
		...
		"%"+lower+"%",
		lower+"%",
		int64(store.PrincipalKindSubAccount),
...
func cleanupOwnedMailData(ctx context.Context, tx *sql.Tx, pid int64, now time.Time) error {
	...
	if _, err := tx.ExecContext(ctx,
		`DELETE FROM state_changes WHERE principal_id = ?`, pid); err != nil {
		return mapErr(err)
	}
	if _, err := tx.ExecContext(ctx,
		`DELETE FROM audit_log WHERE principal_id = ?`, pid); err != nil {
		return mapErr(err)
	}
	...
	queueRows, err := tx.QueryContext(ctx,
		`SELECT body_blob_hash FROM queue WHERE principal_id = ?`, pid)
	if err != nil {
		return mapErr(err)
	}
	...
	queueRows.Close()
	if _, err := tx.ExecContext(ctx,
		`DELETE FROM queue WHERE principal_id = ?`, pid); err != nil {
		return mapErr(err)
	}
...
func checkNoOwnedMailingLists(ctx context.Context, tx *sql.Tx, pid int64) error {
	rows, err := tx.QueryContext(ctx,
		`SELECT posting_address FROM mailing_list WHERE owner_id = ? ORDER BY posting_address`, pid)
	if err != nil {
		return mapErr(err)
	}
	defer rows.Close()
...
func (m *metadata) DeletePrincipal(ctx context.Context, pid store.PrincipalID) error {
	...
	return m.runTx(ctx, func(tx *sql.Tx) error {
		...
		subRows, err := tx.QueryContext(ctx,
			`SELECT id FROM principals WHERE parent_principal_id = ?`, int64(pid))
		if err != nil {
			return mapErr(err)
		}
		var subIDs []int64
		...
		// jmap_states, and sub-principals (parent_principal_id); mailboxes
		// cascades to messages. Blob refs stay (GC sweeps later). grants
		// rows referencing the deleted principal as subject (including
		// mailbox ACL grants, epic #210) are NOT cascaded -- subject_id
		// carries no FK by design (#182), so a grant naming a deleted
		// grantee simply becomes inert (principal ids are monotonic and
		// never reused); granted_by rows referencing this principal as
		// the granting actor are SET NULL, not deleted.
		res, err := tx.ExecContext(ctx,
...
func (m *metadata) SetOIDCProviderAuthzTrusted(ctx context.Context, providerName string, trusted bool) error {
	return m.runTx(ctx, func(tx *sql.Tx) error {
		res, err := tx.ExecContext(ctx,
...
// For a mailbox that does not itself carry MailboxAttrJunk or
		...
		WHERE mm.mailbox_id = ?
		  AND (
		    ...
		    OR NOT EXISTS (
		      ...
		      WHERE mm2.message_id = mm.message_id AND (mb2.attributes & ?) != 0
		    )
		  )`
	mask := int64(store.MailboxAttrTrash | store.MailboxAttrJunk)
	var total, unread int64
	if err := m.s.db.QueryRowContext(ctx, q, int64(mailboxID), int64(mailboxID), mask, mask).Scan(&total, &unread); err != nil {
...
func (m *metadata) CountThreads(ctx context.Context, mailboxID store.MailboxID) (int64, int64, error) {
	...
	var total, unread int64
	if err := m.s.db.QueryRowContext(ctx, q, int64(mailboxID), int64(mailboxID), mask, mask).Scan(&total, &unread); err != nil {
		return 0, 0, fmt.Errorf("storesqlite: count threads: %w", err)
	}
	return total, unread, nil
}
...
func (m *metadata) ReleaseSnooze(ctx context.Context, msgID store.MessageID, mailboxID store.MailboxID) (store.ModSeq, error) {
	...
	err := m.runTx(ctx, func(tx *sql.Tx) error {
		...
		if _, err := tx.ExecContext(ctx, `
			UPDATE message_mailboxes
			   SET keywords_csv = ?, modseq = ?, snoozed_until_us = NULL, wake_mailbox_id = NULL
			 WHERE message_id = ? AND mailbox_id = ?`,
		...
		if snoozedUs.Valid {
			if _, err := tx.ExecContext(ctx, `
				UPDATE messages SET snooze_woke_at_us = ?, snooze_woke_for_us = ? WHERE id = ?`,
				usMicros(now), snoozedUs.Int64, int64(msgID)); err != nil {
				return mapErr(err)
			}
		}
		return appendStateChange(ctx, tx, store.PrincipalID(pid),
