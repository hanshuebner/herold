...
func (m *metadata) SearchAdminMessages(ctx context.Context, filter store.AdminMessageFilter) ([]store.AdminMessageHit, error) {
	...
	if filter.BeforeReceivedUs != 0 {
		conds = append(conds, "m.received_at_us < ?")
		args = append(args, filter.BeforeReceivedUs)
	}
	if !filter.DateFrom.IsZero() {
		conds = append(conds, "m.received_at_us >= ?")
		args = append(args, usMicros(filter.DateFrom))
	}
	if !filter.DateTo.IsZero() {
		conds = append(conds, "m.received_at_us < ?")
		args = append(args, usMicros(filter.DateTo))
	}
	if filter.Sender != "" {
		conds = append(conds, "lower(m.env_from) LIKE lower('%'||?||'%')")
		args = append(args, filter.Sender)
	}
	if filter.Recipient != "" {
		conds = append(conds, "lower(m.env_to) LIKE lower('%'||?||'%')")
	...
	if len(filter.Domains) > 0 {
		...
		conds = append(conds,
			"lower(substr(p.canonical_email, instr(p.canonical_email, '@') + 1)) IN ("+
	...
	if err != nil {
		return nil, fmt.Errorf("storesqlite: SearchAdminMessages: %w", mapErr(err))
	}
	defer rows.Close()

	var out []store.AdminMessageHit
	for rows.Next() {
		hit, err := scanAdminMessageHit(rows)
		if err != nil {
			return nil, fmt.Errorf("storesqlite: SearchAdminMessages scan: %w", err)
		}
		out = append(out, hit)
	}
...
func scanAdminMessageHit(row interface {
	...
	if spamConfidence.Valid {
		c := spamConfidence.Float64
		hit.SpamConfidence = &c
	}
	if spamReason.Valid {
		r := spamReason.String
		hit.SpamReason = &r
