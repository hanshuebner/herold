...
//
// Return-path handling (re #181 analysis, see issue comment): the
// envelope sender is preserved unchanged, matching the #63 Sieve
// redirect behaviour already in production. This is the current,
// intentional default — not an oversight — documented in full in the
// #181 issue comment alongside the SRS alternative and why it is not
// adopted (yet).
//
...
// permanent delivery failure occurs. This is a herold-local DSN
// generated by the queue's own retry/failure bookkeeping — it does not
// require decoding any bounce the destination MTA might separately
// send, since that would go directly to MailFrom's own provider.
func (sess *session) queueForward(
	...
	envID, err := q.Submit(ctx, queue.Submission{
		...
		MailFrom:    sess.envelope.mailFrom, // preserve original envelope sender (see doc comment)
