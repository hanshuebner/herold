...
func (m *metadata) RetargetAliasesByAddress(ctx context.Context, localPart, domain string, newTargetPrincipal store.PrincipalID) error {
	...
	return m.runTx(ctx, func(tx pgx.Tx) error {
		_, err := tx.Exec(ctx, `
			UPDATE aliases SET target_principal = $1
			 WHERE local_part = $2 AND domain = $3 AND target_principal IS NOT NULL`,
...
func (m *metadata) InsertOIDCProvider(ctx context.Context, p store.OIDCProvider) error {
	...
	return m.runTx(ctx, func(tx pgx.Tx) error {
		_, err := tx.Exec(ctx, `
			...
			strings.Join(p.Scopes, ","), p.AutoProvision, p.AutoProvisionDomain,
			p.AuthzTrusted, usMicros(now))
		if err != nil {
...
func (m *metadata) GetOIDCProvider(ctx context.Context, name string) (store.OIDCProvider, error) {
	...
	err := row.Scan(&p.Name, &p.IssuerURL, &p.ClientID, &p.ClientSecretRef,
		&scopes, &p.AutoProvision, &p.AutoProvisionDomain, &p.AuthzTrusted, &createdUs)
	if err != nil {
...
func (m *metadata) InsertMailbox(ctx context.Context, mb store.Mailbox) (store.Mailbox, error) {
	...
	err := m.runTx(ctx, func(tx pgx.Tx) error {
		...
		if err := tx.QueryRow(ctx, `
			...
			VALUES ($1, $2, $3, $4, $5, 1, 0, $6, $7, $8, $9, $10, $11) RETURNING id`,
			...
			string(mb.Disposition), priority).Scan(&id); err != nil {
...
func (m *metadata) DeleteMailbox(ctx context.Context, id store.MailboxID) error {
	...
	return m.runTx(ctx, func(tx pgx.Tx) error {
		...
		err := tx.QueryRow(ctx,
			`SELECT principal_id, priority FROM mailboxes WHERE id = $1`, int64(id)).Scan(&pid, &curPriority)
		if err != nil {
			return mapErr(err)
		}
...
func scanMessageRow(row rowLike) (store.Message, error) {
	...
	msg.FailedImageCount = failedImageCount
	msg.FailedImageState = failedImageState
	msg.RetryableFailedImageCount = retryableFailedImageCount
...
func (m *metadata) loadMailboxes(ctx context.Context, msg *store.Message, mailboxID store.MailboxID) error {
	...
	for rows.Next() {
		...
		var keywords string
		var snoozedUs *int64
		var wakeMailboxID *int64
		var receivedTo string
		if err := rows.Scan(&mid, &mbox, &uid, &modseq, &flags, &keywords, &snoozedUs, &wakeMailboxID, &receivedTo); err != nil {
			return mapErr(err)
		}
		...
		if wakeMailboxID != nil {
			w := store.MailboxID(*wakeMailboxID)
			mm.WakeMailboxID = &w
		}
...
func (m *metadata) SetMessageFailedImages(ctx context.Context, msgID store.MessageID, count int, stateJSON string, retryableCount int, reason string) error {
	res, err := m.s.pool.Exec(ctx,
		`UPDATE messages SET failed_image_count = $1, failed_image_state = $2,
		   retryable_failed_image_count = $3, failed_image_reason = $4 WHERE id = $5`,
...
func (m *metadata) AddMessageToMailbox(ctx context.Context, msgID store.MessageID, mailboxID store.MailboxID) (store.UID, store.ModSeq, error) {
	...
	err := m.runTx(ctx, func(tx pgx.Tx) error {
		...
		return pgRemoveTrashMembershipsAndReplay(ctx, tx, int64(msgID), int64(mailboxID), pid, now)
	})
	if err != nil {
		return 0, 0, err
	}
...
// pgRemoveTrashMembershipsAndReplay deletes every membership msgID
// holds in a Trash-attributed mailbox other than excludeMailboxID (the
...
// #460).
func pgRemoveTrashMembershipsAndReplay(
	ctx context.Context, tx pgx.Tx, msgID, excludeMailboxID int64, pid int64, now time.Time,
) error {
	rows, err := tx.Query(ctx, `
		SELECT mm.mailbox_id FROM message_mailboxes mm
		  JOIN mailboxes mb ON mb.id = mm.mailbox_id
		 WHERE mm.message_id = $1 AND mm.mailbox_id != $2 AND (mb.attributes & $3) != 0`,
		msgID, excludeMailboxID, int64(store.MailboxAttrTrash))
	if err != nil {
		return err
	}
	// Drain into a slice before issuing further statements on tx: pgx
	// pins the connection while a Rows cursor is open.
	var trashIDs []int64
	for rows.Next() {
		var mbID int64
	...
	return pgReplayPretrashMailboxes(ctx, tx, msgID, now)
}
...
func (m *metadata) MoveMessage(ctx context.Context, msgID store.MessageID, fromMailboxID, targetMailboxID store.MailboxID) error {
	...
	return m.runTx(ctx, func(tx pgx.Tx) error {
		...
		var snoozedUs, wakeMB *int64
		var srcReceivedTo string
		err := tx.QueryRow(ctx, `
			SELECT flags, keywords_csv, snoozed_until_us, wake_mailbox_id, received_to
			  FROM message_mailboxes WHERE message_id = $1 AND mailbox_id = $2`,
			int64(msgID), int64(fromMailboxID)).Scan(&srcFlags, &srcKeywords, &snoozedUs, &wakeMB, &srcReceivedTo)
		if errors.Is(err, pgx.ErrNoRows) {
			return store.ErrNotFound
		}
		if err != nil {
...
func (m *metadata) ListOIDCProviders(ctx context.Context) ([]store.OIDCProvider, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&p.Name, &p.IssuerURL, &p.ClientID, &p.ClientSecretRef,
			&scopes, &p.AutoProvision, &p.AutoProvisionDomain, &p.AuthzTrusted, &createdUs); err != nil {
			return nil, mapErr(err)
		}
		if scopes != "" {
			p.Scopes = strings.Split(scopes, ",")
...
func (m *metadata) SetOIDCProviderAutoProvision(ctx context.Context, providerName string, enabled bool, domain string) error {
	return m.runTx(ctx, func(tx pgx.Tx) error {
		res, err := tx.Exec(ctx,
			`UPDATE oidc_providers SET auto_provision = $1, auto_provision_domain = $2 WHERE name = $3`,
			enabled, domain, providerName)
		if err != nil {
			return mapErr(err)
		}
		if res.RowsAffected() == 0 {
...
func (m *metadata) SetOIDCProviderAuthzTrusted(ctx context.Context, providerName string, trusted bool) error {
	return m.runTx(ctx, func(tx pgx.Tx) error {
		res, err := tx.Exec(ctx,
			`UPDATE oidc_providers SET authz_trusted = $1 WHERE name = $2`,
			trusted, providerName)
		if err != nil {
			return mapErr(err)
		}
		if res.RowsAffected() == 0 {
...
func (m *metadata) SetMailboxDisposition(ctx context.Context, mailboxID store.MailboxID, disposition store.MailboxDisposition) error {
	...
	return m.runTx(ctx, func(tx pgx.Tx) error {
		...
		res, err := tx.Exec(ctx,
			`UPDATE mailboxes SET disposition = $1, updated_at_us = $2 WHERE id = $3`,
...
func rankedMailboxPriorities(ctx context.Context, tx pgx.Tx, principalID store.PrincipalID, excludeID int64) ([]mailboxPriorityEntry, error) {
	rows, err := tx.Query(ctx,
		`SELECT id, priority FROM mailboxes
		   WHERE principal_id = $1 AND priority IS NOT NULL AND id != $2
...
func applyDenseMailboxPriorities(ctx context.Context, tx pgx.Tx, principalID store.PrincipalID, ids []int64, oldOf map[int64]*int64, now time.Time) error {
	for i, id := range ids {
		np := int64(i)
		old := oldOf[id]
		if old != nil && *old == np {
		...
		if _, err := tx.Exec(ctx,
			`UPDATE mailboxes SET priority = $1, updated_at_us = $2 WHERE id = $3`,
...
func (m *metadata) ReorderMailboxPriority(ctx context.Context, principalID store.PrincipalID, mailboxID store.MailboxID, newRank *int) error {
	...
	return m.runTx(ctx, func(tx pgx.Tx) error {
		...
		var curPriority *int64
		if err := tx.QueryRow(ctx,
			`SELECT principal_id, priority FROM mailboxes WHERE id = $1`, int64(mailboxID)).
		...
		if newRank == nil && curPtr != nil {
			if _, err := tx.Exec(ctx,
				`UPDATE mailboxes SET priority = NULL, updated_at_us = $1 WHERE id = $2`,
				usMicros(now), int64(mailboxID)); err != nil {
				return mapErr(err)
			}
			if err := appendStateChange(ctx, tx, principalID,
...
func (m *metadata) CountPinnedMailboxes(ctx context.Context, principalID store.PrincipalID) (int, error) {
	var n int
	err := m.s.pool.QueryRow(ctx,
		`SELECT COUNT(*) FROM mailboxes WHERE principal_id = $1 AND disposition = $2`,
	...
	err := m.runTx(ctx, func(tx pgx.Tx) error {
		...
		if _, err := tx.Exec(ctx, `
			UPDATE message_mailboxes
			   SET keywords_csv = $1, modseq = $2, snoozed_until_us = $3, wake_mailbox_id = $4
			 WHERE message_id = $5 AND mailbox_id = $6`,
			strings.Join(kws, ","), int64(modseq), snoozedArg, wakeArg, int64(msgID), int64(mailboxID)); err != nil {
			return mapErr(err)
		}
		if _, err := tx.Exec(ctx, `
			UPDATE mailboxes SET highest_modseq = $1, updated_at_us = $2 WHERE id = $3`,
...
func (m *metadata) RecordMailboxArrival(ctx context.Context, msgID store.MessageID, mailboxID store.MailboxID) (store.ModSeq, error) {
	...
	err := m.runTx(ctx, func(tx pgx.Tx) error {
		...
		var highest int64
		if err := tx.QueryRow(ctx,
			`SELECT highest_modseq FROM mailboxes WHERE id = $1`, int64(mailboxID)).Scan(&highest); err != nil {
			return mapErr(err)
		}
		modseq = store.ModSeq(highest + 1)
		if _, err := tx.Exec(ctx, `
