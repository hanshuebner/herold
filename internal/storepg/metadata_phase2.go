...
func scanQueueItemPG(row pgx.Row) (store.QueueItem, error) {
	...
	err := row.Scan(&id, &principalID, &mailFrom, &rcptTo, &envID,
		...
		&idempKey, &createdUs, &headerOverlay, &messageID)
	if err != nil {
...
func (m *metadata) EnqueueMessage(ctx context.Context, item store.QueueItem) (store.QueueItemID, error) {
	...
	err := m.runTx(ctx, func(tx pgx.Tx) error {
		...
		err := tx.QueryRow(ctx, `
			...
			idempKey, usMicros(item.CreatedAt), item.HeaderOverlay, item.MessageID).Scan(&id)
		if err != nil {
			return mapErr(err)
		}
...
func (m *metadata) ListQueueItems(ctx context.Context, filter store.QueueFilter) ([]store.QueueItem, error) {
	...
	if len(filter.MessageIDs) > 0 {
		...
		for i, id := range filter.MessageIDs {
			placeholders[i] = fmt.Sprintf("lower($%d)", pos)
			args = append(args, id)
			pos++
		}
		where = append(where,
...
func (m *metadata) EarliestNextAttempt(ctx context.Context) (time.Time, bool, error) {
	...
	err := m.s.pool.QueryRow(ctx, `
		SELECT MIN(next_attempt_at_us)
		  FROM queue
		 WHERE state IN ($1, $2)`,
		int32(store.QueueStateQueued), int32(store.QueueStateDeferred)).Scan(&us)
	if err != nil {
...
// shared design note: SETACL/admin-REST own exactly the manual
// (GrantProvenanceLocal / GrantProvenanceACLMigration) rows for a grantee;
// an idp:<provider> row is owned by authz.ReconcileIdP (epic #188) and is
// never read or written here. GetMailboxACL coalesces every provenance row
// for a grantee into one rendered entry (OR of rights).
...
func (m *metadata) GetMailboxACL(ctx context.Context, mailboxID store.MailboxID) ([]store.MailboxACL, error) {
	...
	for rows.Next() {
		...
		if !seen {
			...
			if grantedBy != nil {
				na.GrantedBy = store.PrincipalID(*grantedBy)
			}
			byKey[k] = &na
...
func (m *metadata) GetJMAPStates(ctx context.Context, pid store.PrincipalID) (store.JMAPStates, error) {
	...
	err := m.runTx(ctx, func(tx pgx.Tx) error {
		...
		err := tx.QueryRow(ctx, `
			...
			&seenAddr, &internalizeStatus, &fileShare, &imapImport, &emailBulkJob, &updatedUs)
		if err != nil {
			return mapErr(err)
		}
...
func scanJMAPIdentityPG(row pgx.Row) (store.JMAPIdentity, error) {
	...
	if avatarBlobHash != nil {
		out.AvatarBlobHash = *avatarBlobHash
	}
		...
		tag, err := tx.Exec(ctx, `
			UPDATE jmap_identities
			   SET name = $1, reply_to_json = $2, bcc_json = $3,
			       ...
			       avatar_blob_hash = $7, avatar_blob_size = $8,
			       xface_enabled = $9, updated_at_us = $10
			 WHERE id = $11`,
			row.Name, replyTo, bcc,
			row.TextSignature, row.HTMLSignature, sig,
			avatarHash, row.AvatarBlobSize,
			row.XFaceEnabled, usMicros(now), row.ID)
		if err != nil {
			return mapErr(err)
		}
		if tag.RowsAffected() == 0 {
	...
	err = m.runTx(ctx, func(tx pgx.Tx) error {
		_, err := tx.Exec(ctx, `
			...
			ON CONFLICT (message_id) DO UPDATE SET
			  ...
			  spam_decisive_signal_match = COALESCE(EXCLUDED.spam_decisive_signal_match, llm_classifications.spam_decisive_signal_match),
	...
	return m.runTx(ctx, func(tx pgx.Tx) error {
		tag, err := tx.Exec(ctx,
			`UPDATE llm_classifications
			   SET spam_verdict = $1, spam_model_verdict = NULL, spam_decisive_signal_match = NULL
			 WHERE message_id = $2`,
			verdict, int64(msgID))
		if err != nil {
			return mapErr(err)
...
func (m *metadata) BatchGetLLMClassifications(ctx context.Context, msgIDs []store.MessageID) (map[store.MessageID]store.LLMClassificationRecord, error) {
	...
	for rows.Next() {
		...
		rec := store.LLMClassificationRecord{
			...
			SpamDecisiveSignalMatch: spamDecisiveSignalMatch,
