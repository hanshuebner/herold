...
| ID | Requirement |
|----|-------------|
| REQ-AND-AUTH-20 | The client honours the same session responses as the Suite: `session_expired` and `session_revoked` (`REQ-AS-10`, `REQ-AS-34`) transition to forced-login. TOTP is collected where the server asks for it: herold's `/oauth2/authorize` page requests the 6-digit code in the Custom Tab as a second pass over the same form, and the debug-build device-token fallback answers a `step_up_required` response with a 6-digit field and resubmits once. A `step_up_required` 403 on an authenticated call raises a native six-digit sheet, which posts the code to `POST /api/v1/auth/step-up` (server `REQ-AUTH-79`, issue #357) for both credential kinds; the elevation lives on the credential, so the refused request goes out again once on the token already held. A wrong code and a code whose window has passed keep the sheet up with the server's refusal, and a window that lapses raises the sheet again on the next elevated call. Dismissing the sheet abandons the operation. |
...
| REQ-AND-AUTH-22 | The client surfaces the account's active sessions (Suite `REQ-AS-30..33`) in settings, reading `GET /api/v1/auth/credentials`, including this device, with remote-revoke. This device is the entry the server marks `is_current` for the token the call carried (server issue #356); the family id it is keyed by survives every refresh rotation, so the id read right after the code exchange stays this install's handle. Revoking this device's own grant signs the app out. The screen also mints app passwords for other mail clients, which is a self-service operation the server elevates, so it is where the step-up sheet is exercised. |
