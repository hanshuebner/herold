...
| ID | Requirement |
|----|-------------|
| REQ-AND-AUTH-20 | The client honours the same session responses as the Suite: `session_expired` and `session_revoked` (`REQ-AS-10`, `REQ-AS-34`) transition to forced-login. TOTP is collected where the server asks for it: herold's `/oauth2/authorize` page requests the 6-digit code in the Custom Tab as a second pass over the same form, and the debug-build device-token fallback answers a `step_up_required` response with a 6-digit field and resubmits once. `POST /api/v1/auth/step-up` now accepts a bearer session (device token or OAuth2 access token) and elevates that credential for the same window a cookie session gets (server `REQ-AUTH-79`, issue #357); a native step-up sheet over a bearer session has a server surface to answer, but the sheet itself is not yet implemented (`../notes/parity-matrix.md` § Server gaps). |
