...
| REQ-AND-NAV-32 | The lane the reader picks holds across a sync that adds or reorders lanes; a lane that the server takes away returns the selection to the `primary` lane, and keeps their pick, so the lane coming back - its category holding inbox mail again - takes the selection with it. The row draws through a lane set changing under the selection: the selection and the tabs the row measured are read from two states, so the indicator draws over the tab the row measured and skips a frame that has none (issue #447). |
...
| REQ-AND-NAV-51 | The fold begins at the citation and never above it. An attribution line written over several nodes - Thunderbird spreads "Am ... schrieb `<a href="mailto:...">`address`</a>`:" over a text node, a link and the colon after it - is read as one line, and a reply Thunderbird writes into the citation-prefix div ahead of that line renders outside the fold. An element whose leading content is the sender's own text does not start a fold: the quoted material inside or after it does, so an attribution in a shape the heuristic does not know costs the reader a chip above the quote, never their own message behind one. Leading content counts as the sender's in the region the document opens its quoting with, and only while no citation stands anywhere ahead of that region, at its own level or any level above - a region under a citation holds the quoted message throughout, leading reply and all. A citation-prefix element is exempt: it is a label naming the attribution rather than a container holding a message, so a reply written into it ahead of the attribution is the sender's whatever the body reads above. What else the body carries above or below the citation does not move that boundary (issue #432). |
...
| REQ-AND-NAV-12 | Back leaves the composer the same way its close control does (issue #371). A composer the reader changed since it opened is kept as a draft - to the server's Drafts mailbox, through the outbox with no connection (`02-offline-and-sync.md` REQ-AND-SYNC-21) - and the screen behind offers to discard it. One left as it opened closes silently and writes nothing: a reply, a forward and a reopened draft all open with an address, a subject and a body in them, so what decides is the difference from what the composer was seeded with, not whether its fields are blank. |
