...
| `05-categorisation` — pinned tabs and bundled rows | presentation | done (milestone 4, #399, #404); the lanes are the server's `Mailbox.disposition` and `Mailbox.priority`: pinned labels are the tabs in priority order, bundled ones collapse to one row, `daily`/`weekly`/`filed` stay out of the stream, and a message in several categories shows once under the highest-priority one. A category the account holds no label for takes `pinned` by default - the classifier's `derivedCategories` in the server's order first, then the categories the mail carries - and an uncategorised message falls to the `primary` category (REQ-CAT-03) | #327, #399, #404 |
...
| In-app bug reporter (shake to report, bundle mailed to self) | REQ-AND-SYS-50..53 | done (#407); the phone-side counterpart of the herold-triage browser panel. The drawer, every overflow and a shake raise one sheet; the capture (window PNG, route, build, sync and outbox state, push state, log ring) is taken before the sheet opens and sent through the outbox to the user's own address in the drop layout `herold bug-fetch` expands (#408), filed under a "Bug reports" label the client creates. Typing is optional: an undescribed report is named by its route and time and says so in `report.json`, and the description is added on the desktop. The suite has no counterpart - the browser panel is an extension, not Suite code |
