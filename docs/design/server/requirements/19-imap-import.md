...
| REQ-IMAP-IMP-27 | When the upstream advertises both NOTIFY (RFC 5465) and IDLE, the worker issues `NOTIFY SET` to watch all of the account's personal mailboxes — `SELECTED` with the full event set plus `PERSONAL` with `MessageNew` / `MessageExpunge` / `FlagChange` — and then holds IDLE as in REQ-IMAP-IMP-20. The server pushes events for every watched mailbox during IDLE, so new mail, expunges, and flag changes in non-INBOX folders wake the sync loop at the same latency as INBOX (REQ-IMAP-IMP-22), rather than waiting for a coincidental INBOX event. Watch-mode preference order is **NOTIFY+IDLE > IDLE (INBOX-only, REQ-IMAP-IMP-20) > NOOP-poll (REQ-IMAP-IMP-23)**; NOTIFY is selected only when IDLE is also available, since IDLE is its real-time delivery channel. An upstream that advertises NOTIFY but rejects the `SET` with `NO` / `BAD` falls back to INBOX-only IDLE for that connection and logs once; the worker does not flip to `errored` for a rejected `SET`. |
| REQ-IMAP-IMP-28 | A `NOTIFICATIONOVERFLOW` response (the server has stopped tracking notifications for the connection) is treated as a wake that triggers a full folder re-sync (the same `syncAllFolders` round a normal wake runs), reconciled through the standard dedup and cursor path (REQ-IMAP-IMP-30/34) so no message is lost or duplicated. |
| REQ-IMAP-IMP-29 | The live per-account status surface (REQ-IMAP-IMP-65) exposes the active watch mode (`notify` / `idle` / `poll`) alongside the connection mode (`single` / `dual`, REQ-IMAP-IMP-21). Gmail upstreams do not advertise NOTIFY and remain on IDLE plus the Gmail label path (REQ-IMAP-IMP-53); the NOTIFY benefit applies to standards-compliant multi-folder upstreams (Dovecot, Cyrus, Courier). |
...
| REQ-IMAP-IMP-36 | **Own-address determination for spam classification (re #396, third round).** Herold cannot enumerate every address an upstream mailbox accepts mail at from the account's owning Identity alone — a shared organisational mailbox may also receive mail at aliases (`info@`, `vorstand@`) the Identity does not name. Each `imapimport_account` therefore carries an operator-configured `own_addresses` list (admin REST + `herold imapimport own-addresses set/list`) and a `learned_addresses` set discovered from the `Delivered-To` / `X-Original-To` headers of messages the upstream has already accepted into the account: learned incrementally on every freshly ingested message, and backfilled once, at the first session after the worker is upgraded to support this, from the account's already-imported mail. Both sets feed `spam.ResolveOwnAddresses` (REQ-FILT, the classifier's `recipient_not_own` signal) alongside the principal's identities and aliases. An account with neither a configured list nor a completed learning pass is **own-address-incomplete**: REQ-FILT's decisive-signal resolution MUST NOT let `recipient_not_own` from such an account contribute to a Ham-to-Spam resolution, alone or combined with another signal — an incomplete set makes the signal unreliable, and an unreliable decisive signal produces false positives (the shape this requirement was written to close: transactional mail to an unrecognised organisational alias resolved to spam). The `learned_addresses` set is capped at 256 entries, deduplicated: once the cap is reached, already-learned addresses are kept and no further address is admitted, so an upstream that stamps a distinct `X-Original-To` on every message (a rewriting mailing-list relay, a misbehaving upstream) cannot grow the set without bound. |
| REQ-IMAP-IMP-108 | **Junk-wins and Archive-wins precedence over multi-mailbox-on-dedup (re #303, #376).** REQ-IMAP-IMP-51's "a message in K upstream folders gets K herold mailbox memberships" is qualified for INBOX: a dedup hit mapped to INBOX is redirected instead of placed there, and no INBOX membership is created, when (a) the message already carries a membership in a Junk-attributed or an Archive-attributed mailbox, or (b) the message's thread already has an Archive member and no member of the thread carries an Inbox membership at placement time. The redirected placement (the existing Junk/Archive membership, or a newly-created Archive membership for case (b)) is what the account's `message_state` row for that upstream (folder, uid) addresses (`herold_mailbox_id`); the suppressed INBOX target is recorded separately (`mapped_mailbox_id`) so a later upstream `\Deleted` in that folder can still be reconciled. This precedence is unconditional — it applies to every dedup hit mapped to INBOX, not only a copy of a message the principal sent — so a message that is independently present in both a real upstream Junk/Archive folder and INBOX still converges on the Junk/Archive placement only. |
...
| REQ-IMAP-IMP-42 | **(Decision 5 — supersedes the prior HLC wording.)** Conflict resolution is **upstream-authoritative**. The worker keeps `last_synced_flags` per `imapimport_message_state` row (REQ-IMAP-IMP-34). The reconcile is scoped to the herold **message**, not to one row: a Message-ID dedup hit (a To copy and a Cc copy of the same message, REQ-IMAP-IMP-30) folds several upstream copies onto one herold message, each with its own row, and every row for the message is reconciled together in one pass (re #435). For each row, compare three states: herold-now, upstream-now (that row's own upstream copy), last-synced (that row's own baseline). A row with an outstanding herold-side change whose own upstream copy has not independently moved gets herold's value pushed to it; this runs independently per row, so a local read reaches every upstream copy of the message, and `last_synced_flags` advances together on every row that was successfully pushed. A row whose own upstream copy moved (whether or not herold also changed — a genuine conflict) proposes applying its upstream value to herold; an upstream-side change only wins when no row for the message has an outstanding herold-side change. When at least one row's own upstream copy has moved, herold gets one coherent outcome computed from **every row's current upstream flags for the message** — not only the rows that moved this round: `\Seen` / `\Flagged` wins if set on **any** row's current upstream copy, whether or not that row itself moved this pass. A row that is already caught up and still shows a flag set keeps contributing that flag to the vote; excluding a merely-unchanged row from the vote would let an unrelated sibling's independent drift drag an already-confirmed read back to unseen (re #435, follow-up). Concretely: `\Seen` propagates from herold-\Seen to unseen only when **every** row's current upstream copy is unseen; it is retained if **any** row's current upstream copy is `\Seen`. The identical rule applies to `\Flagged`. `\Deleted` / expunge follows its own per-row path (REQ-IMAP-IMP-43/44) and is not part of this vote. A best-effort herold→upstream STORE that fails (connection drop, rate limit) leaves that row's `last_synced_flags` unchanged so the push is retried on the next reconcile. |
...
| REQ-IMAP-IMP-109 | **`$seen` durability for a message the principal sent (re #316, generalised in #376).** A message the principal sent — any current membership in a `\Sent`-role mailbox, or a From naming one of the principal's own identities — carries no "unread from a correspondent" meaning in any mailbox it is filed into. The upstream-authoritative reconcile of REQ-IMAP-IMP-42 therefore excludes `\Seen` from ever being cleared on such a message: this applies to every current membership of the message (not only the membership the triggering `message_state` row addresses) and on every path that reconciles flags (the per-folder down-sync poll and the change-feed-driven write-back). `last_synced_flags` on the reconciled row records the forced `$seen` state, so a later reconcile does not see it as a spurious conflict against a stale upstream-only baseline. |
