...
| REQ-IMAP-IMP-51 | Mailbox mapping for gmail uses the same locale-aware label table as the takeout importer (REQ-IMPORT-10..14). Gmail's UI labels are localised; the IMAP folder names are NOT (gmail always serves IMAP folder names in English: `Inbox`, `Sent Mail`, etc.). **Implementation limitation (2026-06-07):** the original wording assumed per-message labels arrive as an `X-Gmail-Labels:` header — but that header is a Takeout/Vault *export* artifact and is NOT present in messages FETCHed over IMAP. Over IMAP, labels are only available via the `X-GM-LABELS` FETCH data item (X-GM-EXT-1), which the pinned `emersion/go-imap/v2 v2.0.0-beta.8` client cannot fetch (its FETCH parser rejects unknown msg-att names). Therefore v1 realises the All-Mail-canonical *body* optimisation (REQ-IMAP-IMP-50 — each body fetched once, per-label folders skipped) but per-label *placement* is **deferred**: All Mail messages land in INBOX until `X-GM-LABELS` fetching is available (newer go-imap or a custom FETCH item). The worker retains a best-effort `X-Gmail-Labels:` header parser that activates for any source that does include the header. |
