...
- **REQ-FLOW-33** Per-recipient envelope annotation. Every fan-out mailbox row records the **envelope RCPT TO** that produced it (one canonicalised address). The column is `received_to: TEXT NOT NULL` on the join row between message and mailbox (or equivalent in the per-backend schema). This is the address the upstream MTA gave us at `RCPT TO`, post-alias-expansion, in the form actually accepted by the local listener — not the `To:` / `Cc:` header from the message body, which may differ (BCC delivery, mailing-list rewrites, or any alias the local user has subscribed under). Stored per REQ-STORE-*.
- **REQ-FLOW-34** Render-time header injection. When the server returns the raw RFC 822 body of a message (JMAP `Email/get { properties: ["bodyStructure", "headers"] }`, IMAP FETCH BODY[], `/jmap/download/...`), it MUST prepend a synthetic `X-Herold-Recipient: <received_to>` header to the rendered bytes. The header is generated from the per-recipient `received_to` column (REQ-FLOW-33), NOT stored inside the persisted blob — keeping the blob dedup invariant across fan-out (REQ-FLOW-30). The header is herold-internal and informational; clients that don't care MUST tolerate it. Web-side counterpart: `../../web/requirements/02-mail-basics.md` § Reply identity selection consumes it.
- **REQ-FLOW-35** Outbound never carries `X-Herold-Recipient`. The outbound submission path strips the header from the user-provided message before envelope rewrite, DKIM signing, or queue insertion. Rationale: the header is per-recipient, makes sense only in inbound context, and would leak local routing metadata if relayed. The strip is unconditional and silent (no error to the user); auditing the strip event is optional and not a v1 requirement.
