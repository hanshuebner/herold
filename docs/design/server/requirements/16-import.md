...
| ID | Requirement |
|----|-------------|
| REQ-IMPORT-01 | Accepted input forms: a single `.tgz` / `.tar.gz`, a single `.zip`, a **multipart `.zip` set** (the N independent zip files Takeout's "split archive into pieces" option emits), **or** a directory tree with the layout produced by extraction. The importer auto-detects archive form by content (tar magic, zip magic, presence of multiple zip parts, directory) — no `--format` flag. See REQ-IMPORT-90..99 for archive-reader semantics. |
...
| REQ-IMPORT-04 | The importer is **resumable**. State (last-imported `Message-ID` + position within the logical mbox stream — see REQ-IMPORT-95 for the archive-format-aware cursor encoding) is checkpointed every N messages (default 200) into a per-job row in the metadata store. Re-running the same job ID continues; re-running with a different ID starts over. |
...
| | - `Category: <Primary\|Social\|Promotions\|Updates\|Forums>` -> the matching `$category-*` keyword. The five Gmail categories map 1:1 to the five default categories herold ships (REQ-FILT-210). An imported assignment carries `rule` provenance (REQ-FILT-206), so a later re-categorisation does not silently overwrite what Takeout asserted. |
...
| REQ-IMPORT-45 | `Weiterleitungsadressen.json` (Gmail forwarding addresses) is **imported as data, not as policy**. The list is recorded on the import job for the operator's review. The importer does **not** automatically activate forwarding (principle of least surprise: imported state is data, not auto-policy). The Suite settings UI (REQ-SET-IMPORT-3, see `web/requirements/20-settings.md`) offers a one-click "create forwarding rule for these" affordance backed by Sieve `redirect`. |
...
| ID | Requirement |
|----|-------------|
| REQ-IMPORT-60 | CLI: `herold import gmail --principal <email> [--archive <path> \| --archive-glob <glob> \| --directory <path>] [--locale <tag>] [--dry-run] [--resume <job-id>] [--no-settings] [--no-mail]`. Exactly one of `--archive`, `--archive-glob`, `--directory` is required. `--archive-glob` matches the parts of a multipart `.zip` set (e.g. `takeout-20260101T*-*.zip`); the importer reads them in lexical order. |
...
| REQ-IMPORT-62 | REST: `POST /api/v1/import/jobs` with body `{principal, source: {archive_path \| archive_paths \| directory_path \| upload_id \| upload_ids}, locale?, dry_run?, no_settings?, no_mail?}` returns `{job_id, status: "queued"}`. `archive_paths` / `upload_ids` accept a multipart `.zip` set in lexical order (REQ-IMPORT-93). Admin scope required (REQ-AUTH-SCOPE-*). Public listener does **not** expose this; end-user-driven imports go through the self-service surface (REQ-IMPORT-70). |
...
| REQ-IMPORT-71 | Upload size limit on the public listener: 20 GiB by default, operator-configurable (`appconfig: import.max_archive_size_bytes`). For multipart `.zip` uploads the limit applies to the **sum** of part sizes, not per-part (REQ-IMPORT-98). Larger imports require the operator to use the CLI / admin REST. The limit is advertised in the Suite UI before the user picks a file. |
...
## Archive readers (.tgz, .zip, multipart .zip)

*(Added 2026-05-08 alongside REQ-REPL-* and REQ-IMAP-IMP-*: the original
takeout the importer was built against was a single `.tgz`; new users have
turned up with `.zip` and with multipart `.zip` sets, and operator-driven
"unzip first, then point me at the directory" is awkward. The importer
must consume the artefacts Takeout produces in the form Takeout produces
them.)*

The supported archive shapes have meaningfully different seek and
streaming properties, but the importer's mbox-and-settings contract is
consistent across them. Operators MAY pre-extract to a directory
(REQ-IMPORT-01); they are never required to.

| ID | Requirement |
|----|-------------|
| REQ-IMPORT-90 | The importer accepts each of (a) a single `.tgz` / `.tar.gz`, (b) a single `.zip`, (c) a **multipart `.zip` set**, (d) a directory of pre-extracted contents directly. No "unzip first" step is required. Auto-detection: tar magic at offset 257, PK zip magic at offset 0, multiple zip parts in the supplied directory or matching the supplied glob, or a plain directory containing `Takeout/`. Ambiguity (e.g. a directory containing both a `.zip` and a pre-extracted `Takeout/`) is an error. |
| REQ-IMPORT-91 | **`.tgz` ingestion.** `.tgz` / `.tar.gz` archives are read as a forward-only stream (`gzip.Reader` over the input → `tar.Reader`). The importer makes a single pass: settings JSONs encountered before the mbox are buffered to a small per-job temp directory (≤ 64 MiB; bounded by REQ-IMPORT-81 / REQ-IMPORT-82); the mbox entry, when reached, is fed straight into the streaming mboxrd parser (REQ-IMPORT-20) without intermediate materialisation. For inputs that are regular files the importer MAY take a second pass to pick up settings emitted *after* the mbox; for non-seekable inputs (stdin, pipe) only the canonical Takeout layout (settings before mbox) is supported and a clear error is emitted otherwise. |
| REQ-IMPORT-92 | **`.zip` ingestion.** `.zip` archives are read via Go's `archive/zip` random-access reader. The central directory is parsed once; entries are then opened by name. The mbox entry's content reader is fed directly into the mboxrd parser without copying to a side file. **Zip64 MUST be supported** — Takeout produces Zip64 above 4 GiB, and the maintainer's own takeout was Zip64. Encrypted entries are rejected with `archive-encrypted-entry`; deflate, store, and stored-zstd are accepted. |
| REQ-IMPORT-93 | **Multipart `.zip` is a collection of independent self-contained archives, NOT a spanned/segmented archive.** Google's "split into pieces" option produces N self-contained zip files (e.g. `takeout-20260101T000000Z-001.zip`, `…-002.zip`, …); each part has its own central directory and can be opened in isolation. The importer iterates parts in lexical order. **PKWARE spanned/segmented zip (multi-volume `.z01` / `.z02` / `.zip`) is explicitly NOT supported**; the importer detects spanned-zip headers and refuses with `archive-spanned-zip-not-supported` rather than producing partial output. |
| REQ-IMPORT-94 | Within a multipart set, the canonical `Takeout/Gmail/<localized-mbox-name>.mbox` entry appears in **exactly one** part — Google's split-archive output does not slice individual files across part boundaries. The importer scans every part's central directory once to locate the mbox-bearing part and the parts that contain settings JSONs; it then reads the mbox from its single part and merges settings entries across parts. If the same path appears in two parts, the import fails with `archive-malformed: duplicate entry across parts` rather than silently picking one. If no `.mbox` is found across the part set, the importer treats it as a settings-only takeout (REQ-IMPORT-08). |
| REQ-IMPORT-95 | **Resumable cursor (REQ-IMPORT-04, archive-format-aware).** The checkpoint encodes `(part_index, offset_within_part_entry, last_message_id)`. For `.tgz` and single `.zip`, `part_index == 0`. For multipart `.zip`, `part_index` selects the part whose mbox entry contains the resume position. Re-running with the same job ID and the same archive set resumes; re-running against an archive whose part list / part SHA-256 hashes differ from the persisted manifest aborts with `archive-mismatch` rather than silently skipping or duplicating. The per-part SHA-256 hashes are recorded in the job manifest at job creation time. |
| REQ-IMPORT-96 | **Memory budget across formats** matches REQ-IMPORT-81 (default 512 MiB resident peak). The streaming readers (`gzip.Reader`, `tar.Reader`, `zip.File.Open`) are not allowed to buffer arbitrary amounts; the per-message decoder works on a bounded read window even for multi-GiB messages. |
| REQ-IMPORT-97 | **Disk budget across formats.** `.tgz` requires no archive-shaped temp space (purely streamed). `.zip` requires the input archive to be present as a regular file or as a virtual `io.ReaderAt` over a backing blob with random access (the upload-blob store satisfies this); the importer does NOT extract the entire archive to a side directory. Multipart `.zip` requires all parts to be present concurrently as random-access inputs for the duration of the job. The temp budget for inner per-message work (REQ-IMPORT-82) is unchanged. |
| REQ-IMPORT-98 | **Upload semantics for multipart.** The Suite self-service flow (REQ-IMPORT-70..74) accepts a multipart set by uploading each part as a separate blob, then submitting `source.upload_ids: [blob_id, ...]` in lexical order. The total upload-size limit (REQ-IMPORT-71) applies to the **sum** of part sizes, not per-part. Before accepting parts the Suite UI MUST validate that all uploads share the same Takeout request prefix (`takeout-<timestamp>Z-` is the Google convention) and that the lexical-order numbering is contiguous from 001 to N; gaps or mixed prefixes are rejected client-side with a clear message. |
| REQ-IMPORT-99 | **Test fixtures per format** under `internal/import/gmail/testdata/`: at minimum one `.tgz` fixture, one single-part `.zip` fixture, one multipart `.zip` fixture (3 parts, mbox in middle part, settings split across parts 1 and 3), and one Zip64 fixture. Fixtures are minimal but exercise: settings before mbox in `.tgz`, settings after mbox in `.tgz` (regular-file path), `.zip` with deflate and store entries, `.zip` with one Zip64 entry, multipart with mbox isolated to one part. The archive-reader code path is fuzzed (REQ-IMPORT-80 extension): the tar reader, the zip reader, and the multipart aggregator each get their own fuzz target with corpus seeded from the fixtures. |
