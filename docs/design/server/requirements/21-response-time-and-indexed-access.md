...
| REQ-PERF-INDEX-09 | Date-range filters on `Email/query` (`before` / `after` / `newer_than` / `older_than`) MUST be served by a covering index over `(principal_id, received_at_us DESC, id DESC)` so the SQL planner can scan in already-sorted order and stop at the `Limit` boundary. Without this index, a `before:2009-01-01` style query against the maintainer's 278k-message corpus produces a `USE TEMP B-TREE FOR ORDER BY` plan that materialises the full filtered set before sorting; the resulting wall time exceeds the default 1 s deadline (REQ-PERF-DEADLINE-01) and the user observes `serverFail: method exceeded response deadline` (reported 2026-05-10). The fix is a non-partial index in the next migration; the partial covering index added by REQ-EXTIMG-BG-INTERNAL-82 only matches `internalize_pending = 1` rows and cannot satisfy this query. The index also supports the inbox's no-filter sort path and any future range query keyed on `received_at_us`. |
| REQ-PERF-INDEX-10 | The fast-pushability gate at `internal/protojmap/mail/email/fastquery.go:mergeFilterIntoOpts` MUST flatten a single-level `{operator: 'AND', conditions: [...]}` whose every conjunct is itself flat-pushable. Reported 2026-05-10: `applyTrashJunkExclusion` produced this shape for any default-scoped search, dropping every such query to the slow `listPrincipalMessages` path even though the underlying SQL is trivially fast. Refusal cases are deeper nesting (AND-inside-AND), other operators (OR/NOT), and conflicting predicates (e.g. two conjuncts both setting Before). |
