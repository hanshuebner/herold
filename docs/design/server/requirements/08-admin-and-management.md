...
- Custom role definitions (the 3 roles from REQ-AUTH-60 are the fixed set). **[Superseded by `07-access-control.md`: authorization is resource grants, not custom global roles; the fixed-3-roles framing no longer holds.]**
- Web-based config file editor in v1.
- Delegated admin with scoped permissions (phase 3). **[Lifted by `07-access-control.md`: delegated scoped admin is now in scope and is exactly the grant model; REQ-ADM-307 was already an instance of it.]**
