...
| ID | Requirement |
|----|-------------|
| REQ-AC-50 | Mailbox grants are the shared-mailbox / cross-principal read substrate, and the sole storage for RFC 4314 ACL data (epic #210). A mailbox grant's stored level carries the full RFC 4314 rights letter-set (`l r s w i p k x t e a`) the grant conveys, letter-exact; `mailbox:read`/`write`/`admin` remain named tiers as a convenience projection for non-ACL-wire grants and the UI, expanded to their implied rights on read. `SETACL`/`GETACL`/`MYRIGHTS`/`LISTRIGHTS` (REQ-PROTO-33) read and write these grant rows directly, with no separate ACL table. This realises the phase-2 "schema carries per-mailbox ACL entries" as concrete grant storage, and supersedes the "deferred, separate dimension" framing of REQ-AUTH-63 by giving it a home in this model. |
