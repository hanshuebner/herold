...
- **REQ-OPS-ADMIN-LISTENER-03** A single `herold_public_session` cookie carries both end-user scopes and (when the principal is admin + TOTP-verified) `ScopeAdmin`. The scope check at every admin handler is the enforcement boundary; the single-cookie design eliminates the cross-listener cookie-scope confusion from the previous two-listener model. **[Corrected as stale: see the REQ-OPS-ADMIN-LISTENER-01 note and REQ-AUTH-SCOPE-01 — the single cookie carries end-user scopes only; `ScopeAdmin` is not baked into it, and admin authorization is the elevation record (`07-access-control.md` REQ-AC-11).]**
