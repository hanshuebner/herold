...
- **REQ-AUTH-79** `POST /api/v1/auth/step-up` also accepts a Bearer-authenticated caller — a device token (REQ-AND-AUTH-01) or an OAuth2 access token (REQ-AUTH-71) — so a native client's TOTP step-up sheet has a server surface to answer (issue #357). The elevation record is keyed on whichever credential authenticated the request:
  - Cookie session: the elevation record lives in `session_elevations`, keyed on `session_id`, exactly as REQ-AUTH-74 describes.
  - Bearer caller: the elevation record lives in a parallel `api_key_elevations` table, keyed on the authenticating `api_keys` row id, with the same `{principal_id, elevated_at, idle_deadline, absolute_deadline}` shape and the same idle/absolute windows (REQ-AUTH-ELEV-CONFIG). Deleting the `api_keys` row (credential revocation) cascades to its elevation record, mirroring session revocation's cascade in REQ-AUTH-74.
  - REQ-AUTH-78's self-service gate (`requireSelfServiceElevation`) applies identically to both credential kinds: a TOTP-enrolled principal calling a gated self-service operation over a Bearer credential gets `403 step_up_required` absent an active elevation for that credential, and a principal with no TOTP enrolled proceeds without elevation regardless of credential kind.
  - The admin gate (`requireElevation`) is unaffected: a Bearer caller reaches admin-scoped endpoints via `ScopeAdmin` in the credential, not via an elevation record — REQ-AUTH-79 only extends the self-service gate to Bearer callers.
  - A Bearer step-up request is CSRF-exempt (no ambient credential), identically to every other Bearer-authenticated endpoint; a cookie-session step-up request remains CSRF-checked per REQ-AUTH-74. Wrong-code handling, rate-limiting (`directory.VerifyTOTP`, keyed on principal + source, not credential), and the `enroll_required` no-TOTP response are shared code paths, so both credential kinds get identical behavior.
...
- **REQ-AUTH-SCOPE-04** API key scope is set at creation time and immutable (rotate to change). The `herold apikey create` command (REQ-ADM-03) takes `--scope` as a comma-separated list of allowed values; default is `[mail.send]` (the most common transactional-app shape per REQ-SEND-30). Admin scope on an API key requires `--allow-admin-scope` to acknowledge the operator-side risk; cookies are the recommended path for human admin access. `--scope bug-reports` (issue #416) mints a key that is refused by every handler other than the bug-reports REST surface (REQ-ADM-320..324); no acknowledgement flag is required since the scope carries no broader privilege. `herold bug-fetch` is the intended holder of such a key.
