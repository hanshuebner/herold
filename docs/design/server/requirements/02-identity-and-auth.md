...
- **REQ-AUTH-42** TOTP (REQ-AUTH-40) serves as the step-up credential for admin and destructive operations (REQ-AUTH-74). It does not gate the initial login flow. IMAP/SMTP submission with app passwords bypasses TOTP by design.
...
- **REQ-AUTH-74** TOTP step-up elevation for admin and destructive operations. Any request requiring admin scope returns `403` with `{"type": "step_up_required", "elevation_scope": "admin"}` in the RFC 7807 body when no valid elevation record exists for the session. A correct TOTP code submitted to `POST /api/v1/auth/step-up` (body: `{totp_code}`, rate-limited per session and per source IP, CSRF-checked) creates a server-side elevation record `{session_id, principal_id, elevated_at, expires_at: now + elevation_ttl}`. Subsequent admin-scoped requests within the elevation window proceed without re-prompting. The elevation TTL is operator-configurable under `[server.ui] session.elevation_ttl` (default `"15m"`). A wrong TOTP code returns `401`; after five consecutive wrong codes within a 5-minute window the session's elevation attempts are locked for 5 minutes. Revoking a session (REQ-AUTH-77) also invalidates all elevation records for that session. Elevation grants and failures are audit-logged with `action="auth.step_up"`, `outcome=success|failure`, `actor=principal/<id>`.

- **REQ-AUTH-75** The server surfaces effective session expiry to the client. `GET /api/v1/auth/whoami` MUST include `{..., session_idle_deadline: <ISO-8601 UTC>, elevation_expires_at: <ISO-8601 UTC | null>}` in its response body. `session_idle_deadline` is the current rolling idle deadline; it advances on every authenticated request. Clients use these fields to proactively trigger forced re-login before a 401 arrives (web REQ-AS-10, REQ-AS-11) and to display elevation-countdown UI (web REQ-AS-24).
...
- **REQ-AUTH-JSON-LOGIN**: `POST /api/v1/auth/login` accepts `{email, password}` (unauthenticated, rate-limited per source IP). On success it issues `herold_admin_session` (HttpOnly, Secure, SameSite=Strict, Path=/) and `herold_admin_csrf` (non-HttpOnly, Secure, SameSite=Strict, Path=/) cookies and returns `{principal_id, email, scopes:[...], session_idle_deadline: <ISO-8601 UTC>}`. The issued session carries only end-user scopes (REQ-AUTH-SCOPE-01); admin operations require a subsequent TOTP step-up (REQ-AUTH-74, REQ-AUTH-SCOPE-03). TOTP is not evaluated at login. On bad credentials the endpoint returns 401. Failed login attempts MUST be audit-logged with `action="auth.login"`, `outcome=failure`, `subject="email:<attempted-email>"`, and a `message` distinguishing the failure mode (per REQ-ADM-300, REQ-ADM-303); successful logins are audit-logged with `actor=principal/<id>`, `subject="principal:<email>"`, `outcome=success`.
