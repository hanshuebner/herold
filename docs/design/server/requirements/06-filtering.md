...
- **REQ-FILT-13** **One classifier plugin, one call, both answers (Wave 4.3, ADR-0002).** The server asks `mail.classify(message, context) -> { verdict, confidence, reason, category }` of a `classifier`-type plugin once per message, after SMTP DATA and on IMAP pull. `context` carries the principal, the recipient domain, and the principal's stored category set (REQ-FILT-210); a category the plugin names outside that set is ignored and logged. `category` is optional (REQ-FILT-230). How the plugin answers — one model call, several, rules, no model at all — is the plugin's business; herold ships `herold-spam-llm` as the first-party implementation and operators may replace it with any conforming plugin. A `spam`-type plugin (the pre-Wave-4.3 contract, `spam.classify(message) -> { verdict, confidence, reason }`) is accepted for one release and treated as a classifier that never returns a category.
- **REQ-FILT-14** **Structured signals and server-side resolution of a contradictory verdict (re #396).** The response contract carries `spam_signals` / `ham_signals` (short snake_case trait names) alongside `verdict`/`score`/`reason`. A `Ham` verdict whose `spam_signals` name at least one signal is flagged `Inconsistent` on the transparency record regardless of what follows. Separately, the server resolves — not merely flags — that contradiction to `Spam` (or `Suspect` below an operator-configured confidence) when `spam_signals` matches a **decisive-signal rule**: an entry from the operator-configurable set (default: `unsolicited_bulk_marketing`, `unauthenticated_sender`+`dmarc_fail` together, `phishing`, `recipient_not_own`+`bulk_list_relay` together), matched case-insensitively; the plugin's own verdict is preserved on the transparency record (`model_verdict`) alongside the applied one. Two of the decisive facts are server-computed, not left to the model to notice or name: `recipient_not_own` (no To/Cc address is in the request's `own_addresses`) and `bulk_list_relay` (`List-Id` / `List-Unsubscribe` / `Precedence: bulk` or `list` present). `recipient_not_own` is never decisive alone — only combined with another decisive signal — and never contributes to a decisive match at all when the own-address set for the message's account is known incomplete (REQ-IMAP-IMP-36): an incomplete set makes the signal unreliable, and its reliability is exactly what determines whether ignoring it (false negative on a genuinely unrecognised recipient) is safer than trusting it (false positive on a legitimate unrecognised-alias recipient).
...
- **REQ-FILT-213** *Withdrawn (Wave 4.3, issue #304).* No server-side compilation of category definitions: the classifier plugin answers the category question directly, in the same `mail.classify` call as the spam verdict (REQ-FILT-13). What a plugin does internally — compile a definition into rules, call a model, both — is not the server's concern.
- **REQ-FILT-214** **The structural fallback categoriser (Wave 4.3, ADR-0002; refined issue #491).** The server keeps one small categoriser for itself. `forums` requires a discussion-list marker — `List-Post`, or `Precedence: list`, or `List-Id` together with `List-Post`. `List-Id` or `List-Unsubscribe` without a discussion-list marker is one-way bulk mail: `updates`, or `promotions` when `Precedence: bulk` is also present. `Auto-Submitted` present, with none of the above, → `updates`. Nothing else — no rule language, no configuration, no model. It runs only where the classifier plugin's `category` is empty (REQ-FILT-13) or no classifier plugin is installed, and the plugin's category always wins when it gives one. This is what keeps the tab strip working on a herold with no classifier configured.
- **REQ-FILT-215** *Withdrawn (Wave 4.3, issue #304).* There is no server-side rule language for categories to share; REQ-FILT-213's compilation step does not exist.
...
The backtest is what turns a natural-language definition into a repeatable operation: before a ruleset is activated, the user sees what it would do to their own mail. Because scoring is a compiled ruleset in pure Go, the per-message cost is microseconds and the reason to sample largely disappears — the cost that remains is reading each message's header block out of the store.

- **REQ-FILT-226** **The backtest is full, not sampled.** A candidate ruleset is evaluated against **every** message in the principal's mailbox, bounded by a configurable limit (default 200 000 messages). When the mailbox exceeds the limit, the backtest evaluates the most recent N by `receivedAt`. It runs as a background job with progress reporting; the preview is presented on completion. Evaluation is deterministic: the same ruleset over the same mailbox yields the same diff.
- **REQ-FILT-227** **No silent truncation.** When the limit binds, the preview states its coverage explicitly — "backtested the most recent 200 000 of 480 000 messages". A diff whose scope is not stated reads as complete when it is not, and the count is the thing the user is about to make a decision on.
- **REQ-FILT-228** **The diff is a change list, not a quality measure.** It reports what *would move* relative to the currently active rulesets. It does NOT report precision, recall, or accuracy for categorisation, because there is no ground truth to compute them against: outside the user's own corrections, a message's category was asserted by the machine, so the backtest compares a new ruleset against an old one and not against truth. The suite MUST NOT present a quality score for a category. (Spam is different — user Junk moves are real labels, and REQ-FILT-217's threshold recalibration uses them.)
- **REQ-FILT-229** **Leakage guard.** During a backtest the evaluator MUST NOT see any feature derived from a previous classification of the same message — `$category-*` keywords, `$Junk`, or the `X-Spam-*` headers herold itself added (REQ-FILT-03). A ruleset scored against its own prior output predicts it perfectly and measures nothing. The feature extractor masks these inputs in backtest mode, and the masking is tested.
