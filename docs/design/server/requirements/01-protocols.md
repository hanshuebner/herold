...
- **REQ-PROTO-128** `Email.mailboxIds` MUST be a multi-valued set per RFC 8621 §1.6.1 — an `Email` can live in multiple mailboxes simultaneously within the same account. `Email/get` returns every mailbox the message is in; `Email/set update` accepts a full replacement set or PatchObject patches (`mailboxIds/<id>: true | null`); `Email/set destroy` removes the message from every mailbox; `Mailbox/set onDestroyRemoveEmails` removes references from the destroyed mailbox only — messages still in other mailboxes survive. Backed by the M:N storage model in REQ-STORE-36..38. Re-running the JMAP test suite after this lands MUST clear the four deferred failures listed in `test/interop/README.md` "Known deferred failures" (`email/get-mailbox-ids`, `email/set-update-add-mailbox`, `email/set-update-remove-mailbox`, `email/set-destroy-removes-from-all-mailboxes`).
...
- **REQ-PROTO-49** MUST implement the JMAP snooze extension. Modern mail clients (Apple Mail, Fastmail, etc.) expose "snooze until <time>" as a first-class user action; the de-facto JMAP shape is a `$snoozed` keyword on `Email` plus a `snoozedUntil` (UTCDate) extension property, with a server-side wake-up timer that clears the keyword + the property when `snoozedUntil <= now`. Phase 2.
  ...
  - Wake marker (issue #469): on release the same tick also stamps two nullable `messages` columns, `snooze_woke_at_us` (the release instant) and `snooze_woke_for_us` (the deadline that fell due), exposed as read-only `Email` properties `snoozeWokeAt` and `snoozeWokeFor` (both UTCDate) so a client can say why a message came back and until when it was set. Both are `null` when no reminder has fired, or once the marker is cleared. Message-level, not per-mailbox — a wake often adds the message to a mailbox other than the one the snooze was set from, and the marker must stay visible there. The clearing rules mirror the reminder-ending rule above: the marker clears when the message gains `$seen` (through any path — `Email/set`, IMAP `STORE`, an importer) and when the message is snoozed again; a deliberate cancel of a still-pending snooze (`snoozedUntil: null` before the deadline) leaves the marker untouched, since nothing has woken. Each clearing transition appends its own `state_changes` row, same as the wake itself.
...
Per `docs/design/web/notes/server-contract.md` § Email reactions. Capability `https://netzhansa.com/jmap/email-reactions`. Phase 2.
...
Per `docs/design/web/requirements/25-push-notifications.md` and `docs/design/web/notes/server-contract.md` § Web Push. Capability `https://netzhansa.com/jmap/push` (advertised when the deployment has VAPID configured). Phase 1 — herold ships push delivery as part of v1 because the suite v1 depends on it.
