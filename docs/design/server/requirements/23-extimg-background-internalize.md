...
| REQ-EXTIMG-BG-91 | *Superseded in part by 17-external-images.md REQ-EXTIMG-RETRY-01..08 (2026-07-09, issue #162): a failed Internalize attempt does not automatically requeue the message for another background worker pass, but the user can now trigger a targeted per-message retry via `Email/retryImages`. What remains out of scope: an operator-driven bulk "re-flag every message with failed images" maintenance command — the user-triggered retry covers the reported gap without it. |
