...
| REQ-EXTIMG-BG-03 | A new store method `ListMessagesWithInternalizePending(ctx, afterMessageID, limit) ([]MessageID, error)` returns up to `limit` MessageIDs in ascending order with ID > afterMessageID. The worker maintains an in-memory cursor across polls so a single pass through the backlog converges. After a full pass with no remaining matches the cursor resets to 0; subsequent imports raise the flag on new rows and the next poll picks them up. |
