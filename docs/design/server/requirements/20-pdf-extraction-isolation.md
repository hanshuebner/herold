...
| REQ-PDFEX-21 | Each child has a **memory cap** applied via `RLIMIT_AS` (Linux) or `RLIMIT_DATA` (macOS / BSDs) in `SysProcAttr.Setrlimit`-equivalent. Default 1 GiB, configurable via `appconfig: fts.extract.pdf.max_memory_bytes`. The kernel kills the child on overrun; the parent observes the non-zero exit and records the failure. **Note:** on macOS `RLIMIT_AS` is unenforced; the wall-clock timeout (REQ-PDFEX-20) is the only reliable bound. The configuration MUST still be plumbed so Linux operators get the hard cap. |
...
| REQ-PDFEX-41 | Failures are observable via `herold_fts_attachment_extracted_total{format="pdf",outcome="<o>"}` with the closed vocabulary `{"ok","error","timeout","oom","crash","oversized","truncated_attachment","truncated_message","binary_missing"}`. The wrapper distinguishes timeout from crash by checking `ProcessState.ExitCode()` and signal cause; OOM is detected by a child killed with SIGKILL with no parent timeout having fired. |
...
| REQ-PDFEX-71 | The official herold container image MUST include `poppler-utils` (Alpine: `apk add poppler-utils`; Debian: `apt-get install -y poppler-utils`). The image-build script asserts the binary is present at the expected path. |
...
| REQ-PDFEX-112 | The cursor reconciliation is automatic: every message processed under the in-process extractor that hit a PDF is currently indexed *without* PDF text (because the stopgap skipped it). Once the subprocess wrapper ships, the operator can run `herold admin diag fts reindex --kind pdf-attachments` (new subcommand) to re-extract every message that has a PDF attachment, against the new wrapper. The subcommand walks the messages table, replays the FTS path for matching rows, and bumps the same metrics. |

## Open questions remaining

- **Container image policy.** The `:slim` herold image variant (no JS bundle, no docs) — does it ship poppler-utils too, or omit it on the assumption that `:slim` operators want maximum minimality and accept opt-in PDF? Resolution before REQ-PDFEX-71 lands.
- **macOS rlimit alternatives.** `RLIMIT_AS` is unenforced on macOS. Is wall-clock timeout sufficient as the only memory bound for macOS dev / single-tenant deploys, or do we want a separate "macOS-extractor-watchdog" that polls the child's RSS via `proc_pidinfo` and SIGKILLs past a threshold? Resolution before REQ-PDFEX-21's macOS path stops being best-effort.
- **Reindex subcommand scope.** Does REQ-PDFEX-112's `reindex --kind pdf-attachments` need a per-principal scope (some operators may want to retry only one user's messages), or is the corpus-wide form sufficient for v1? Resolution while implementing the subcommand.
