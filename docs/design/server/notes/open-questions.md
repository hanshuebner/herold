...
### Q5. JMAP capability vendor URI scheme — `netzhansa.com` is provisional *(opened 2026-04-27, redirected 2026-04-27 from herold.dev)*

The Phase 1c-3c rename (tabard merge plan) replaced the historical
`https://tabard.dev/jmap/*` capability URIs with
`https://netzhansa.com/jmap/*` across server, client, tests, and docs.
(An interim `herold.dev` placeholder was used during 3c and then
swept to `netzhansa.com` in the same Phase 1 cycle once the operator
domain was settled.) The project has not yet decided whether the
production capability URI should keep `netzhansa.com`, move to a
`urn:netzhansa:` scheme, or move to an
`urn:ietf:params:jmap:vendor:herold:` IANA registration. The current
URI is a placeholder so the wire surface is internally self-consistent;
