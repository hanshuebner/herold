...
| **Chat datatypes (`Conversation`, `Message`, `Membership`)** | `requirements/14-chat.md` REQ-CHAT-01..06 (phase 2). Net-new entity kinds in herold's storage; capability `https://herold.dev/jmap/chat`. Additive on the existing JMAP capability registry and the open entity_kind enum — no migration of existing tables. |
