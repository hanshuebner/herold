...
Goal: a Suite that can read mail.
...
Exit gate: the user can open the client, read their inbox, and archive threads with `e`. The result `Email/changes` after a manual mutation in another client correctly fans out to the suite.
...
Exit gate: the user can run their day from the suite. WF-01 (inbox-zero pass) and WF-05 (compose and send) work end-to-end.

## Phase 3 — extended mail features

The suite-side work that sits on top of phase 2 once core mail is fluent.
...
Herold provides the substrate for all of these; the work is purely suite-side.
...
A locally running herold is the assumed substrate during the suite development and manual testing. The suite's Vite dev server proxies JMAP / EventSource / WebSocket / login / image-proxy requests to that herold (default `http://localhost:8080`; override via `HEROLD_URL` env var). See `apps/suite/README.md`.
