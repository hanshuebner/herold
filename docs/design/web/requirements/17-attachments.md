...
| REQ-ATT-26 | Each rendered inline image is independently downloadable with a single action: a small download button overlays the image on hover and on keyboard focus, and the right-click context menu offers "Save image as…". An inline image the browser cannot decode into a displayed image (detected by a content-type hint for known non-web-renderable formats such as `image/tiff`, and/or a runtime `<img>` decode failure — an `error` event or zero natural dimensions once loading settles) additionally gets a chip in the attachment card strip below the body, with the same chip-level download button as regular attachments, so its original bytes stay reachable even though it never displays. A normally-rendering inline image gets no chip (REQ-MAIL-21). The download path uses the blobId per REQ-ATT-25. (G16; issue #269.) |
...
| REQ-ATT-63 | On a successful `FileShare/set create`, the suite inserts the share into the message body **in both alternatives**: a styled link block into the `text/html` body (filename, human size, type icon, expiry, and the `url` as the anchor) and a plain-text line (`name (size) — url`) into the `text/plain` alternative. The block is inserted at the cursor, or appended above the signature when compose is not focused in the body. The inserted link is ordinary body content — it carries no special markup and survives autosave like any other edit (REQ-DFT-03). The expiry shown is the share's `expiresIn` retention (the lifetime chosen at create, clamped to `max_ttl`, per REQ-SHARE-41) — never a value derived from the still-`pending` share's `expiresAt`, which reflects `pending_ttl` (REQ-SHARE-20), not the chosen retention. Because this block is frozen into the outgoing HTML at attach time, it is the retention the recipient is told, so it must be correct. |
| REQ-ATT-64 | Offloaded files appear in a dedicated **"Shared links"** strip, visually distinct from the attachment chip strip, each row showing: filename, size, type icon, expiry ("expires in 30 days"), an optional download-cap badge, a lock icon when password-protected, and a remove control. The expiry label uses the same retention-based rendering as REQ-ATT-63, through the one shared formatter both surfaces call: a duration is rounded to the nearest day (or, under a day, the nearest hour), never floored — a 48-hour lifetime renders "2 days", not "1 day". Removing a row issues `FileShare/set destroy` and deletes the corresponding link block from both body alternatives. |
| REQ-ATT-65 | Before creating the share the suite MAY offer, behind a small "Options" disclosure on the offer dialog, an expiry selector, an optional download cap, and an optional password. The expiry selector's presets and default are drawn from the `default_ttl_seconds` / `max_ttl_seconds` fields the file-shares session capability advertises (REQ-SHARE-45) — not a client-side constant — so an operator's `max_ttl` reaches the picker and no preset above it is offered. When a password is set, the suite shows the password once with a copy affordance and a reminder that it must be shared out-of-band; the password is write-only and cannot be retrieved later (REQ-SHARE-43). |
...
| REQ-ATT-70a | Each share displays its **message back-reference** (REQ-SHARE-04) so the owner can see why and with whom the file was shared: the source message's subject and recipient list, rendered as e.g. "Shared in 'Q3 report' — to bob@example.com, carol@example.com". When the source message still exists, the subject is a link that opens that message (deep-link via `sourceMessageId`); when it has been deleted, the subject renders as plain text (the snapshot context still shows). A `pending` share, not yet confirmed, shows no back-reference. The compose flow supplies `sourceMessageId`, `sourceSubject`, and `sourceRecipients` on the confirm-on-send update (REQ-ATT-66). |
