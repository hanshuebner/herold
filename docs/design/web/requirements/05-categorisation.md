...
| REQ-CAT-43a | The preview states the backtest's **coverage** whenever it was bounded by the limit — "backtested the most recent 200 000 of 480 000 messages" (REQ-FILT-227). A diff presented without its scope reads as complete when it is not. |
| REQ-CAT-43b | The preview presents a **change list, never a quality score**. The suite does not show precision, recall, accuracy, or a confidence figure for a category, because no ground truth exists to compute one from (REQ-FILT-228). The user's judgement is made by reading messages they recognise, not by trusting a number the server cannot honestly produce. |
