...
The suite's mobile web experience is first-class. Phones and tablets get a complete, idiomatic UI — not a desktop fallback. Native iOS / Android applications remain out of scope (`00-scope.md` NG1 amended); a Progressive Web App (PWA) installable from Safari / Chrome is the deployment shape.
...
This doc describes how suite-shell layout, the mail app, the chat panel, and component patterns adapt across breakpoints, plus the touch-specific interactions and the PWA shell.
...
| REQ-MOB-27 | Native text selection is preserved: long-press on selectable text (subject, body, sender name) uses the platform's native selection menu (Copy / Look up / Share). The suite does not intercept text selection. |
...
| REQ-MOB-32 | Bottom navigation: a fixed bar with up to 3 items — Mail, Chat, Settings (or whichever apps are deployed). Tapping switches suite app context, replacing the active pane. The chat panel doesn't apply on phone (the panel pattern is desktop-only); chat is one of the bottom-nav apps instead. |
...
| REQ-MOB-54 | System back button (Android): pops the current navigation stack entry — close picker → close compose → leave thread → close drawer. The suite listens to `popstate` and manages the History API to align with this behaviour. |
| REQ-MOB-55 | iOS swipe-back gesture: same nav-stack pop. The suite's History API integration handles both. |
| REQ-MOB-56 | Status bar / safe areas: the suite respects `env(safe-area-inset-*)` so notches, home indicators, and rounded-corner cutouts don't clip content. The PWA manifest declares `theme_color` matching the active light/dark theme. |
...
| ID | Requirement |
|----|-------------|
| REQ-MOB-60 | The suite listens to `visualViewport.resize` events and adjusts layout: bottom-anchored elements (compose toolbar, FAB, bottom navigation) reposition above the keyboard. The currently focused input MUST remain visible — never obscured by the keyboard. |
...
| REQ-MOB-63 | The suite does NOT prevent IME composition or autocorrect (typing Japanese, autocorrect on iOS, swipe-typing on Android all work as the platform provides). Compose's ProseMirror editor handles `compositionstart`/`compositionend` correctly. |

## PWA install

| ID | Requirement |
|----|-------------|
| REQ-MOB-70 | The suite ships a Web App Manifest (`/manifest.webmanifest`) referenced from every HTML entry point. Fields: `name`, `short_name`, `start_url`, `scope`, `display: "standalone"`, `theme_color` (light + dark variants via media queries), `background_color`, `icons` (192×192, 512×512 PNG plus 1024×1024 maskable), `categories: ["productivity", "communication"]`. |
| REQ-MOB-71 | App icons follow the design-system colour palette and use a suite mark — design TBD; placeholder geometric icon ships with v1. |
| REQ-MOB-72 | The suite does NOT prompt the user to install. The install affordance is the platform's own (Safari "Add to Home Screen"; Chrome "Install app"). The suite does NOT show a custom install banner. |
...
| REQ-MOB-75 | The service worker handles app updates: when a new version of the suite ships, the worker prompts a soft refresh ("A new version is available — Reload"). Same as a desktop browser tab; mobile users just see the prompt in-app. |
...
| REQ-MOB-82 | Background tab unload behaviour on mobile: iOS Safari and Chrome Android aggressively unload backgrounded tabs to free memory. The suite treats tab restore as a fresh bootstrap (cache rebuilds via JMAP `Foo/get` calls). The user does not see this as a "lost work" state — it's a fresh fetch with the same view state. |
...
| REQ-MOB-91 | The suite's bundle is route-split: the suite shell + mail-app code load eagerly; chat, settings, calendar (when it ships), and contacts (when it ships) lazy-load on first navigation to their routes. Same code-split graph applies to desktop, but mobile feels the savings more. |
...
| ID | Requirement |
|----|-------------|
| REQ-MOB-100 | The suite supports system font scaling: suite-wide type scale is in `rem` units; `<html>` font-size respects the user's system text-size preference (iOS Dynamic Type, Android system font scaling). Layouts reflow at scaled sizes; we don't fix component widths in pixels. |
| REQ-MOB-101 | Screen readers: VoiceOver (iOS) and TalkBack (Android) navigate the suite the same way assistive tech navigates desktop (REQ-A11Y-02). All touch-specific patterns (long-press menus, swipe actions, bottom sheets) are reachable via the screen-reader rotor / explore-by-touch interaction with appropriate announcements. |
...
| REQ-MOB-121 | Coach observation (REQ-COACH-10..15) is paused on phone; mouse-vs-keyboard distinctions don't apply. The user's existing per-action stats are preserved (a desktop user who occasionally uses the suite on phone keeps their accumulated stats from desktop). |
...
- Tablet split-screen multitasking with another app sharing the suite's data. The browser sandbox doesn't enable that anyway; if the user runs the suite alongside another app via OS-level split-screen, both windows of the suite run independently as separate sessions.
