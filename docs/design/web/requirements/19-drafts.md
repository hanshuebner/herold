...
A draft is an `Email` in the user's `drafts` mailbox with the `$draft` keyword. Drafts are server-stored; the suite does not maintain a parallel client-side draft store. Compose state that hasn't yet auto-saved lives in `sessionStorage` for crash recovery only.

## Auto-save

| ID | Requirement |
|----|-------------|
| REQ-DFT-01 | While compose is open and the body is dirty (any field changed since the last save), the suite auto-saves on a debounce of 5 seconds. The 5 s window resets on every keystroke. |
...
| REQ-DFT-51 | On tab reload, the suite reads `sessionStorage`: if any compose state is present, it re-opens those compose windows pre-populated. After successful re-render, the `sessionStorage` entries are kept until the next successful auto-save. |
...
| REQ-DFT-31 | If the server returns `stateMismatch`, the suite pauses auto-save, fetches the current server-side draft via `Email/get`, and presents a non-blocking inline banner in the compose: "This draft was edited from another device. [Take this version] [Use the server version] [Show diff]". |
...
| ID | Requirement |
|----|-------------|
| REQ-DFT-60 | If `EmailSubmission/set` fails (server error, transport error), the suite does NOT remove the draft. The compose window re-opens (or stays open) with the body intact, the `$draft` keyword still set, and a toast: "Send failed: <reason>. The message is saved as a draft." |
