...
│ Inbox        14  │ [ pinned category tabs, max 5 ]                 │
│ Snoozed          ├─────────────────────────────────────────────────┤
│ Important        │                                                 │
...
| ID | Requirement |
|----|-------------|
| REQ-UI-13a | Top of the sidebar: a prominent "Compose" button (also `c` shortcut). The "Herold" brand mark sits above Compose, with a burger-menu button as defined in REQ-UI-13k. |
| REQ-UI-13k | A burger-menu icon button sits at the left end of the brand-mark row in the sidebar. Clicking it opens a small dropdown listing the other suite components the user has access to (REQ-UI-13l..n). Each item is a real `<a href>` so the user's middle-click / Cmd+click / right-click "open in new tab" works as expected; the suite's own click handler also opens the link in a new browser tab via `target="_blank" rel="noopener"`. The current app does not appear in the menu (the user is already in it). The same component appears in every suite SPA — the consumer suite at `/`, the operator admin at `/admin/`, and any future Calendar / Contacts / Chat fullscreen entry points. |
| REQ-UI-13l | Menu entries (in fixed order, hidden when the corresponding gate is false): Mail (`/#/mail`), Calendar (`/#/calendar` — gated on the calendar capability when it exists; cut for v1 visibility), Contacts (`/#/contacts` — gated on the contacts capability; cut for v1 visibility), Chat (`/#/chat` — gated on the chat capability per `08-chat.md`), Server admin (`/admin/`). Each entry shows a small icon and the localised label. |
| REQ-UI-13m | The Server-admin entry is visible only when the user's session carries the `admin` scope (REQ-AUTH-SCOPE-01). For users without admin scope the entry is omitted entirely (no greyed-out / disabled affordance). |
| REQ-UI-13n | The burger-menu dropdown closes on: clicking an entry, pressing Escape, or clicking outside the dropdown. The button has `aria-expanded` and `aria-controls` attributes; the menu uses `role="menu"` with each entry as `role="menuitem"`. Min hit area: `var(--touch-min)`. |
...
| REQ-UI-09e | Below the list header, in the Inbox only: the strip of **pinned** category tabs (at most 5), per `05-categorisation.md` REQ-CAT-11. Other views hide the strip. |
...
| REQ-UI-21b | Per-message right-side controls (collapsed and expanded): attachment indicator (paperclip when present), star (toggles `$flagged`), react button (emoji picker; reactions stored as a server-side `Email.reactions` extension per `02-mail-basics.md` § Reactions). When expanded, also: three-dot more-menu (per-message context menu). The kebab carries the conservative-slice items enumerated in `02-mail-basics.md` § Per-message context menu — the inherently-per-message verbs (download .eml, show original, print this message) plus the verbs whose thread-scoped variant is a poor fit for multi-sender threads (delete this message, mark this message unread, mark unread from here, report spam, report phishing, filter messages like this). Reply and forward live in the fixed reply bar at the thread bottom (REQ-UI-25a), not in the kebab. |
