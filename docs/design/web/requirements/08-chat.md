...
| REQ-CHAT-183 | **`absent`** — neither `present-in-chat` nor `present-elsewhere` holds. That is: the window does not have focus, OR no input event has been observed for `idleThresholdSeconds`. Default `idleThresholdSeconds` is 300 (5 minutes); operator config may override. |
