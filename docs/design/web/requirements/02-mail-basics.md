...
| REQ-MAIL-23 | Reader provides a view/download overlay on every attachment chip. Hovering or focusing an attachment reveals a "View" action (where applicable: image, PDF, plain text) and a "Download" action. Clicking an image attachment opens it in a lightbox; clicking a PDF opens an in-browser preview pane (no native viewer launch). Other types fall through to Download. The lightbox and PDF iframe request the blob with `?disposition=inline` so herold's `/jmap/download/...` endpoint emits `Content-Disposition: inline` (default is `attachment` — without the inline flag the iframe would render blank and the browser would trigger a download). |
| REQ-MAIL-23a | **Gmail-style attachment cards.** Each non-inline attachment renders as a horizontally laid-out card with a square type-icon area (LEFT) and the filename + MIME type + size in helper text (RIGHT). Image attachments under 2 MB show a thumbnail crop in the icon area (`object-fit: cover`); other types render a coloured badge (PDF=red, DOC=blue, XLS=green, ZIP/FILE=grey, IMG=teal) with a 3-letter label. A bottom-right dog-ear triangle decoration echoes the Gmail visual language. On hover or `:focus-within`, a translucent dark overlay reveals icon-only "View" and "Download" affordances; the View action is suppressed for non-previewable types. The card click target opens the lightbox (REQ-MAIL-23); the download anchor calls `stopPropagation` so it does not also fire the card click. |
...
| REQ-MAIL-25 | **Trimmed-quote toggle is localised.** The "Hide trimmed content" / "Zitat ausblenden" label rendered by the rendered-iframe stylesheet's `details.herold-quoted[open] > summary::before` follows the suite's current locale. Implemented by interpolating `t('mail.trimmed.hide')` into the iframe's CSS at sanitize time. |
| REQ-MAIL-26 | **Iframe re-measures when the trimmed quote opens or images load.** The reader iframe is sandboxed `allow-same-origin` (no scripts) and its outer `height` is set from the parent based on the rendered `body.scrollHeight`. The parent listens to `toggle` and `load` events on the iframe's `contentDocument` (capture phase, so any `<details>` inside bubbles up and any nested `<img>` `load` reaches the parent) and re-runs the measurement on `requestAnimationFrame`. Without this listener, opening a trimmed quote leaves the iframe pinned to its initial height and the revealed text falls below the viewport, requiring a manual scroll. |
...
| REQ-MAIL-30a | **Reply on own sent message routes to the original recipients.** When `parent.from[0].email` matches one of the user's `Identity` addresses (case-insensitive), Reply pre-fills `to` with `parent.to` (the people who actually received the message) instead of the user's own From address. Reply-all on own sent messages preserves `parent.to` as the new `To` and `parent.cc` (minus own-identity addresses) as the new `Cc`. The own-message detection requires the `mail.identities` cache to be populated; the suite primes `loadIdentities()` on auth-ready alongside `loadMailboxes()`, and Reply / Reply-all defensively await the load if the cache is still empty when the user clicks. |
| REQ-MAIL-31 | Reply-all populates `to` with the original sender and `cc` with the original `to`+`cc` minus the user's own `Identity` addresses. (See REQ-MAIL-30a for the own-sent-message branch.) |
...
| ID | Requirement |
|----|-------------|
| REQ-MAIL-40 | **Avatar.** Each message header in the thread reader shows a circular avatar to the LEFT of the sender name. v1: when the sender's principal carries no profile picture (REQ-MAIL-44), the avatar renders the sender's first initial on a `--interactive` (Carbon blue-60) background with `--text-on-color`. The same colour palette is used by the chat sidebar avatars so a person reads as the same avatar across mail and chat surfaces. |
| REQ-MAIL-41 | **Date format.** The header date renders in the suite's current locale via `Intl.DateTimeFormat(localeTag(), { weekday:'short', day:'numeric', month:'short', year:'numeric', hour:'2-digit', minute:'2-digit' })`. |
| REQ-MAIL-42 | **Relative-time annotation.** Every message header — collapsed and expanded — appends a parenthesised relative-time annotation to the absolute date, e.g. "Wed, 30 Apr 2026, 12:02 (vor 17 Stunden)" / "(17 hours ago)". The annotation is computed at render time via `Intl.RelativeTimeFormat(localeTag(), { numeric: 'always' })` against six coarse buckets: < 1 minute → `time.justNow` ("just now" / "gerade eben"); < 1 hour → minutes; < 24 hours → hours; < 7 days → days; < 5 weeks → weeks; otherwise months. The bucket selection uses the absolute delta so future-dated messages degrade gracefully. The label is a one-shot per render — no live ticker. |
| REQ-MAIL-43 | **Attachment indicator.** When a message carries at least one non-inline attachment (i.e. `email.attachments` has any part with `disposition !== 'inline'`, falling back to `email.hasAttachment` when the array is absent) the header renders a paperclip glyph (`U+1F4CE`) immediately to the LEFT of the date. Inline images alone — body parts referenced by `cid:` URLs — do NOT trigger the indicator; they are part of the body, not a separate attachment. The icon carries an `aria-label` keyed by `att.headerIcon.label`. |
| REQ-MAIL-44 | **Avatar resolution chain.** The avatar shown at REQ-MAIL-40 is resolved per sender email through a tiered lookup. (1) **Own identity.** When the sender's address matches one of the user's own `Identity.email` values and that identity carries `avatarBlobId` (REQ-SET-03b), the JMAP download URL with `disposition=inline` is used. (2) **Hosted-principal lookup.** A batched `Principal/query { emailExact }` + `Principal/get { properties: ['avatarBlobId'] }` round-trip resolves the avatar of any *other* hosted principal whose `principals.avatar_blob_hash` is set. The herold server stores the canonical default-identity avatar on the principal row (migration 0036) so cross-user lookups always work; restart-safe. (3) **Email metadata** — gated by REQ-SET-17's privacy toggle, default ON, configured by the operator at server level (`appconfig`): (3a) the message's `Face:` header (base64 PNG / JPEG, RFC-style modern X-Face replacement) is decoded into a `blob:` URL; (3b) Gravatar is HEAD-checked at `https://www.gravatar.com/avatar/<sha256>?d=404`. The first hit wins. (4) **Initial-letter fallback.** Results (positive and negative) are cached per-email in `localStorage['herold:avatar:cache']` with a 24-hour TTL so reloads do not re-issue the same network probes. The toggle key is `localStorage['herold:avatar:emailMetadata']`. |
| REQ-MAIL-45 | **Outbound X-Face / Face injection.** When the sending Identity has `xFaceEnabled = true` and `avatarBlobId` is set (REQ-SET-03b), `Email/set` injects a `Face:` header (base64-encoded PNG/JPEG bytes) and an `X-Face:` header (legacy 48×48 1bpp PBM compressed) derived from the avatar blob. The user opts in once per identity via the Suite's settings UI, with a hint that the headers add ~1 KB to every outbound message. Disabling the toggle stops injection but does not remove already-sent headers. |
| REQ-MAIL-46 | **Recipient hover card.** Hovering (or keyboard-focusing) any person reference in an expanded thread message header — the avatar (REQ-MAIL-40), the sender display name, and every recipient entry rendered in the `To:`, `Cc:`, and `Bcc:` rows — opens a Gmail-style contact card in a floating popover anchored to the hovered element. Plain text in the message body is NOT a hover trigger; only structured person references in the header are. The card is dismissed when the pointer leaves both the trigger and the popover, when the popover loses focus, or when the user presses `Escape`. Hover-intent delay: open after 400 ms of sustained hover, close after 150 ms once the pointer leaves; keyboard focus opens the card immediately. Only one card is open at a time; moving to a new trigger cancels the previous card without re-paying the open delay. |
| REQ-MAIL-46a | **Card content.** The card always shows: (1) a large avatar resolved through REQ-MAIL-44's chain; (2) the display name on the first line (the most recently observed `name` from the parsed address, falling back to the local-part when no name is known); (3) the email address on the second line, truncated with an ellipsis when it overflows the card width and accompanied by a copy-to-clipboard button that writes the canonical lower-cased address and shows a transient "Copied" tooltip. When the address resolves to a hosted Principal (REQ-MAIL-44 step 2) or to a JMAP Contact, the card additionally surfaces any phone numbers carried by the matched record, each rendered on its own row with the phone-icon glyph and the type label ("Mobile", "Work", …) localised through the suite's i18n table. |
| REQ-MAIL-46b | **Primary action.** A full-width "Send email" / "E-Mail senden" button is the card's primary action. Clicking it opens a new compose window (REQ-MAIL-10) pre-populated with the address as the sole `To` recipient, no subject, no body. The button is always enabled — every address with a structurally-valid email can receive mail. |
| REQ-MAIL-46c | **Secondary actions.** Three icon-only round buttons sit to the right of the primary action: chat, video call, calendar event. They are enabled only when the address resolves to a hosted Principal on this server (i.e. REQ-MAIL-44 step 2 returned a match) AND the corresponding capability is advertised on the JMAP session — chat gated on `https://netzhansa.com/jmap/chat` (`08-chat.md`), video on `https://netzhansa.com/jmap/calls` (`21-video-calls.md`), calendar on `urn:ietf:params:jmap:calendars`. When a capability is absent or the recipient is not a hosted Principal, the button renders in the disabled visual state (greyed, no hover affordance) and is removed from the keyboard-tab order. Chat opens the existing DM with that Principal, or the new-chat picker (`08-chat.md` REQ-CHAT-01a) pre-filled with the address; video starts a call against that Principal (`21-video-calls.md`); calendar opens the event composer with the address pre-filled as an attendee (`15-calendar-invites.md`). |
| REQ-MAIL-46d | **Contact-state corner action.** The card's top-right corner carries one of two icons depending on whether the address is already a JMAP Contact in the user's primary AddressBook: (a) **not a contact** → an "add contact" person-plus icon. Clicking it issues `Contact/set` to create a new Contact in the primary AddressBook seeded with the captured display name and the canonical email, drops the matching `SeenAddress` row in the same transaction (REQ-MAIL-11l), and updates the card in place — the icon swaps to the edit pencil and the corner reveals the "View detailed view" link (REQ-MAIL-46e) on the next render. (b) **already a contact** → a pencil "edit" icon that routes to the contact's detail surface (`12-workflows.md` contact-detail flow) for editing. The icon carries an `aria-label` keyed by `contact.card.add` / `contact.card.edit`. |
| REQ-MAIL-46e | **View detailed view.** When the address resolves to a JMAP Contact, the card renders a "View detailed view" / "Detaillierte Ansicht anzeigen" link beneath the phone-numbers block. Clicking it routes to the contact's detail surface (the same surface targeted by REQ-MAIL-46d's edit icon, in read mode). The link is absent for non-contact addresses — there is no detail view for a SeenAddress / Principal-only record. |
| REQ-MAIL-46f | **Resolution and caching.** The card resolves the address through the same tiered lookup as REQ-MAIL-44: own Identity → hosted-principal `Principal/query` + `Principal/get` (with `phones` and `avatarBlobId` in the requested properties) → JMAP Contact lookup → email-metadata fallback. The resolution payload is cached per email in the same `localStorage['herold:avatar:cache']` entry the avatar lookup writes (extended to also carry display name, phones, and contact/principal-match flags); a card open against a cached entry is fully populated synchronously and re-validates in the background. The contact-state and phone fields participate in the JMAP state-change push (`Contact/changes`, `Principal/changes` where available) so opening a previously-not-a-contact address right after the user clicked "add contact" reflects the new state without a manual refresh. |
...
| REQ-MAIL-54 | Move-to relocates a thread (or a multi-selection of threads) into a single chosen mailbox. Like archive (REQ-MAIL-51) and delete (REQ-MAIL-52), this is **thread-scoped**: the suite expands every selected list-row email id to its `Thread.emailIds` set before issuing the JMAP `Email/set` update, so every email in each affected thread — the original, every reply, every forward still part of that thread — has its `mailboxIds` replaced with `{<target-id>: true}`. Reply messages do not stay behind in the source folder. Surfaced as the move-to toolbar button (REQ-UI-19b, REQ-UI-51) and as drag-and-drop from the thread list onto a sidebar mailbox (REQ-UI-17). Optimistic with Undo. |
