...
| REQ-SET-03b | **Per-identity profile picture.** The Account section exposes an avatar editor next to "Display name". The user picks an image file (PNG / JPEG / WebP / GIF, ≤ 1 MB after client-side downscaling to a 512×512 max bounding box); the suite uploads via `Blob/upload` and persists the resulting `blobId` on the identity through an extension property — provisional name `Identity.avatarBlobId` (TBD: confirm the wire form against `notes/server-contract.md` before the server side lands). The same blob is rendered on every avatar surface: each thread message header (REQ-MAIL-40), the chat sidebar avatars, and the suite chrome's user-avatar menu. Avatars for *other* principals (incoming messages, chat partners) come from the same source via a server-side directory lookup keyed on the principal's canonical email; v1 falls back to the initial-letter avatar when the lookup misses. Removing a picture clears the field; the server may garbage-collect the blob through normal refcount semantics. | Server-side via the `Identity` extension property; the blob lives in herold's blob store and is fetched through `/jmap/download/...`. |
