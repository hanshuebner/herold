...
| ID | Requirement |
|----|-------------|
| REQ-SET-IDENT-10 | The per-identity edit dialog (extending the existing `IdentityEditDialog.svelte` used today for external-submission setup) hosts the full editor for ONE identity at a time: avatar editor (REQ-SET-03b), display-name field (REQ-SET-03a), signature editor (REQ-SET-03), X-Face opt-in (REQ-MAIL-45), the **Sending (SMTP)** section (REQ-MAIL-SUBMIT-01..06 — the mandatory send path; for external-domain identities it must be probe-verified before the dialog can be saved, REQ-AUTH-EXT-SUBMIT-11/12), and, for external-domain identities, the **optional Receiving (IMAP import)** section (REQ-SET-IMAPIMP-01..05). The two sections make the identity a self-contained "external account": send via SMTP, optionally mirror inbound via IMAP. The dialog is reached from the list by clicking the row or by deep-linking via `?identity=<id>` (REQ-MAIL-SUBMIT-06's re-auth path already uses this). Closing the dialog returns the user to the list with no scroll loss. |
...
| REQ-SET-IDENT-12 | **Remove identity** action lives at the bottom of the dialog, visually de-emphasised, with a confirmation modal: "Remove <name> <<email>>? Mail already sent from this identity is unaffected. This cannot be undone." On confirm the dialog closes and the row disappears from the list (`Identity/set { destroy }`). **When the identity carries imported mail** (it has an IMAP import config that mirrored messages), the modal additionally presents the keep-or-delete-imported-mail choice of REQ-SET-IMAPIMP-04 / REQ-IMAP-IMP-102 (default: keep), since destroying the identity cascades to its import config. The synthesised default identity has no Remove button. |
...
## IMAP import per Identity (v1)

Self-service UI for IMAP client mode, lives in the **Receiving (IMAP
import)** section of the per-identity edit dialog (REQ-SET-IDENT-10),
beneath the mandatory **Sending (SMTP)** section. IMAP import is the
optional inbound half of an external-transport identity; one identity maps
to at most one upstream IMAP account. Server contract:
`../../server/requirements/19-imap-import.md` (REQ-IMAP-IMP-*, per-identity
per its decision 10) + JMAP `IMAPImport/get|set`. **This UI does not exist
yet** — the backend (JMAP + admin REST + CLI) shipped without any suite
surface; these requirements specify the missing surface.

| ID | Requirement |
|----|-------------|
| REQ-SET-IMAPIMP-01 | The Receiving (IMAP import) section appears in the edit dialog **only for external-domain identities** and only when the JMAP session advertises `https://netzhansa.com/jmap/imap-import` (a hosted-domain identity's mail is already local — no section). It shows the identity's import state from `IMAPImport/get`: configured-or-not, state (`enabled` / `disabled` / `errored` / `migrating` / `migrated`), backfill floor, last-success / last-error, and per-folder progress (from the read-only status surface, REQ-IMAP-IMP-65). When unconfigured it shows a single "Set up mail import" affordance. |
| REQ-SET-IMAPIMP-02 | **Set-up form** (in-dialog, not a separate wizard): host / port / TLS mode, username, auth method (`password` / `app_password` / `xoauth2`), the credential (write-only field, never echoed back — REQ-IMAP-IMP-70), a **required** backfill horizon (`30d` / `90d` / `1y` / `all` / custom date — REQ-IMAP-IMP-15), and optional folder mapping (defaults to name-equals-name). Submitting issues `IMAPImport/set create` against the owning identity. The `xoauth2` option is shown only when the operator has registered an OAuth app (REQ-IMAP-IMP-03) and carries the operator-burden note. Host / username default from the identity's SMTP-submission config where the provider is known (e.g. Gmail), so the user rarely re-types them. |
| REQ-SET-IMAPIMP-03 | **Edit + migrate.** The section edits the mapping / horizon / credential and exposes a "Complete migration" action (REQ-IMAP-IMP-90) with an explanation: it pulls the entire mailbox, makes herold authoritative, and retires the upstream connection. The action confirms before committing the state transition. |
| REQ-SET-IMAPIMP-04 | **Remove import — keep-or-delete prompt (REQ-IMAP-IMP-102).** Turning off / removing the IMAP import for an identity — and likewise destroying an identity that has imported mail — opens a confirmation dialog with two explicit choices: (a) **Keep imported mail** (default) — "Removes the connection only; the mail already imported stays in your mailboxes." (b) **Also delete imported mail** — "Permanently delete the N messages imported through this account. This cannot be undone and does not affect the upstream server." The count N is shown (from the provenance label, REQ-IMAP-IMP-100). On confirm the suite issues `IMAPImport/set destroy` with the chosen `deleteImportedMail` flag and surfaces progress for a large purge. This prompt also fires from the identity's own Remove action (REQ-SET-IDENT-12) when the identity carries imported mail. |
