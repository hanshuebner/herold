...
2. **REST (`POST /api/v1/...`).** Bearer-token-authenticated; the
   ...
   at `/api/openapi.json` (REQ-ADM-05) is not yet published; tracked
   under REQ-ADM-05.
...
Not yet wired in Wave 3; the server returns 501 today. The DKIM keypair
is still generated by `herold domain add` internally and is used for
signing -- it just is not yet readable through the CLI surface.
...
```bash
herold principal create user@example.com --password 'change-me-now'
```

Passwords must be at least 12 characters (REQ-AUTH-22). If omitted,
the server generates a random 20-character password and prints it once.

Flags:

- `--password <str>` - explicit password.
...
Unlike `principal show / disable / enable / grant-admin / revoke-admin
/ quota`, the `delete` subcommand requires the numeric id. Look it up
with `herold principal show <email> --json | jq .id`.

```bash
herold principal delete 42
```

### Reset password

Not yet wired; tracked under the protoadmin admin REST surface. Until
then, recover via `herold principal disable` followed by
`herold principal enable` (the re-enable flow resets the lockout), or
use the REST equivalent when available.
...
Not yet wired; tracked under the protoadmin admin REST surface. The
only currently working path to mint an API key is `herold bootstrap`
(one-time admin) or the self-service endpoint
`POST /api/v1/principals/{pid}/api-keys` once that surface is
...
The JSON-Lines SIEM export at `/api/v1/audit-log` is not yet wired;
tracked under REQ-ADM-302. Use `/api/v1/audit?limit=N` (paginated
JSON, the same surface that backs `herold audit list`) until then.

For SIEM ingestion, the audit log will export as JSON-Lines via
...
Not yet wired; the CLI flag mapping does not match the protoadmin
schema (CLI sends `issuer_url`, server expects something else).
Tracked under the protoadmin admin REST surface.
