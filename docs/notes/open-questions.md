...
| 13 | Monorepo tooling — pnpm workspaces, npm workspaces, or Nx? Decide when the second app (tabard-calendar or tabard-contacts) starts and the split actually happens. | repo root | Deferred until split |
| 14 | When does tabard-calendar start? Coordinated with herold's JMAP for Calendars implementation. Until herold ships the capability, the iMIP RSVP path in mail talks to herold's calendars endpoints directly without a sibling-app handoff. | `00-scope.md`, `requirements/02-mail-basics.md`, future `apps/calendar/docs/` | Open — depends on herold roadmap |
| 15 | When does tabard-contacts start? Until then, recipient autocomplete in compose sources from a client-local seen-addresses history (Q9). | `requirements/02-mail-basics.md` REQ-MAIL-11, future `apps/contacts/docs/` | Open — depends on herold roadmap |
| 16 | Cross-app handoff mechanism within the suite — same-origin URL with a path scheme (`/calendar/event/<id>`)? Postmessage between iframes? Shared state in a parent shell? Decide when the second app starts. | `architecture/01-system-overview.md` (eventually) | Deferred until split |
