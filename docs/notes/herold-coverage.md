...
| Chat datatypes (`Conversation`, `Message`, `Membership`) over JMAP | `requirements/08-chat.md`, `architecture/07-chat-protocol.md` | Net-new datatypes in herold. Storage tables, JMAP datatype handlers, capability registration (`https://tabard.dev/jmap/chat`). The forward-compat work in herold's `architecture/05-sync-and-state.md` (open `entity_kind` enum) and the JMAP capability registry pattern means these are purely additive — no schema migrations of existing tables. Herold's `notes/stalwart-feature-map.md` notes Stalwart's chat scope is empty too; this is fresh ground. |
| Chat ephemeral WebSocket at `/chat/ws` | `architecture/07-chat-protocol.md` § Ephemeral channel | New endpoint. Carries typing indicators, presence updates, WebRTC call signaling. Per-user rate limits, heartbeat. Authenticated by the suite session cookie. |
| TURN credential minting | `requirements/21-video-calls.md` REQ-CALL-32, `architecture/07-chat-protocol.md` § TURN credentials | Herold mints short-lived TURN credentials against a coturn shared secret. Operator deploys coturn at the same origin or a closely-coordinated origin (deployment guidance lives in herold's deploy/ docs — TBD). |
| Multi-user presence tracking | `requirements/08-chat.md` REQ-CHAT-60..63 | Server-side state derived from WebSocket connections; pushed to interested peers. Privacy controls (per-user "show as offline" mode). |
