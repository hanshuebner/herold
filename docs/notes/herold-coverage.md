...
| Image proxy at `<origin>/proxy/image?url=...` (strips tracking headers, size cap) | `requirements/13-nonfunctional.md` REQ-SEC-07; without it external images stay blocked unconditionally | New endpoint on herold's HTTP listener. Slot into `requirements/12-http-mail-api.md` or a new requirement. Implementation is small (fetch + header-rewrite + size cap). |
