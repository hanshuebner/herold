/**
 * Issue #469 (work item 3) -- mailbox-row marker for a message whose
 * reminder just fell due.
 *
 * The MailView row shows a small `<span class="woke-badge">` carrying an
 * icon and the "Reminder" text when the backend's per-message
 * `snoozeWokeAt` field is set. The marker follows the fold: the server
 * clears `snoozeWokeAt`/`snoozeWokeFor` when the message gains `$seen` or
 * is snoozed again, and each clearing is an `Email` change-feed row, so
 * the badge disappears once the row's data refreshes -- no reload needed.
 */
...
const { mailMock, routerState } = vi.hoisted(() => {
  ...
  const EMAIL_WOKEN = {
    ...
    keywords: {} as Record<string, true | undefined>,
    from: [{ name: 'Alice', email: 'alice@example.com' }],
    to: [{ name: 'Bob', email: 'bob@example.com' }],
    subject: 'Woken message',
    preview: 'A reminder just fell due for this message.',
    receivedAt: '2026-05-09T10:00:00Z',
    hasAttachment: false,
    snoozedUntil: null as string | null,
    snoozeWakeMailboxId: null as string | null,
    snoozeWokeAt: '2026-05-09T09:00:00Z' as string | null,
    snoozeWokeFor: '2026-05-09T09:00:00Z' as string | null,
  };
  ...
  const EMAIL_PLAIN = {
    ...
    preview: 'This message was never snoozed.',
    receivedAt: '2026-05-09T11:00:00Z',
    hasAttachment: false,
    snoozedUntil: null as string | null,
    snoozeWakeMailboxId: null as string | null,
    snoozeWokeAt: null as string | null,
    snoozeWokeFor: null as string | null,
  };
  ...
  const mailMock = {
    ...
    listEmailIds: [EMAIL_WOKEN.id, EMAIL_PLAIN.id],
    get listEmails() {
      return [EMAIL_WOKEN, EMAIL_PLAIN];
    },
    mailboxes: mbxMap,
    get customMailboxes() {
      return [];
    ...
    emails: new Map([
      [EMAIL_WOKEN.id, EMAIL_WOKEN],
      [EMAIL_PLAIN.id, EMAIL_PLAIN],
    ]),
...
vi.mock('../lib/i18n/i18n.svelte', () => ({
  t: (key: string, args?: Record<string, unknown>): string => {
    const map: Record<string, string> = {
      ...
      'mail.row.woke.tooltip': `Back because a reminder set for ${String(args?.time ?? '')} fell due.`,
    };
    return map[key] ?? key;
  },
  localeTag: () => 'en',
...
describe('MailView on-wake marker (issue #469)', () => {
  beforeEach(() => {
    routerState.folder = 'inbox';
    mailMock.listFolder = 'inbox';
  });

  it('renders the woke-badge on the row whose email has snoozeWokeAt set', () => {
    const { container } = render(MailView);
    const rows = container.querySelectorAll('.thread-row');
    expect(rows.length).toBeGreaterThanOrEqual(2);
    const wokenRow = rows[0]!;
    const badge = wokenRow.querySelector('.woke-badge');
    expect(badge).not.toBeNull();
    expect(badge?.textContent).toContain('Reminder');
    expect(badge?.getAttribute('title')).toContain('fell due');
  });

  it('does not render the woke-badge on a row whose email was never snoozed', () => {
    const { container } = render(MailView);
    const rows = container.querySelectorAll('.thread-row');
    const plainRow = rows[1]!;
    const badge = plainRow.querySelector('.woke-badge');
