 ...
 * applySearchTrashJunkExclusion. The helper wraps the parsed filter with
 * inMailboxOtherThan: [<trash-id>, <junk-id>] when those mailboxes
 * exist for the principal.
 */

import { describe, it, expect } from 'vitest';
import { applySearchTrashJunkExclusion } from './store.svelte';
...
describe('applySearchTrashJunkExclusion (REQ-SRC-06)', () => {
  ...
  it('splices inMailboxOtherThan into a flat before-filter without wrapping in AND', () => {
    ...
    m.set('mb-trash', mb('mb-trash', 'Trash', 'trash'));
    m.set('mb-junk', mb('mb-junk', 'Junk', 'junk'));
    const out = applySearchTrashJunkExclusion({ before: '2009-01-01T00:00:00Z' }, m);
    expect(out).not.toHaveProperty('operator');
    expect(out).toEqual({
  ...
  it('returns the filter unchanged when neither role exists', () => {
    ...
    expect(applySearchTrashJunkExclusion(filter, m)).toBe(filter);
  });
  ...
  it('wraps an operator-shaped parsed filter with AND', () => {
    ...
    const out = applySearchTrashJunkExclusion(inner, m);
    expect(out).toEqual({
      operator: 'AND',
