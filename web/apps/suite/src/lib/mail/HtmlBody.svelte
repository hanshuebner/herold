<script lang="ts">
   ...
   *     bare in-document fragment links, which stay same-document so
   *     clicking one scrolls the reader instead of navigating away
   *     (issue #293): see sanitize.ts's `fragmentDocumentUrl` doc comment.
   *     The scroll itself is the browser's own native cross-frame
   *     fragment-navigation behaviour -- when the target has no room to
   *     scroll within the iframe's own (always fully auto-sized, see the
   *     wheel-forwarding comment below) viewport, the browser's "scroll
   *     the indicated part of the document into view" algorithm carries
   *     on into the parent browsing context and scrolls the outer thread
   *     container itself. No script runs to do this; adding one to
   *     "help" double-scrolls on top of the native behaviour and
