/**
 * Tiny in-app i18n, mirroring the suite's mechanism
 * (`web/apps/suite/src/lib/i18n/i18n.svelte.ts`) so both SPAs share the
 * same `t()` API and locale model (re #180).
 *
 ...
 * placeholder replacement lets us interpolate counts and strings without
 * pulling in a full ICU formatter.
 *
 * Locale persistence is handled by the settings store; this module just
 * surfaces a reactive cell that the settings store writes into.
 * Detection: settings.hydrate() seeds from localStorage; if absent, we
 * sniff `navigator.language` once and pick 'de' for any de-* tag, English
 * otherwise.
 */
 ...
 * matches the active locale. Both supported locales pin to a 24-hour,
 * day-month-year region, matching the suite (issue #23).
 */
