<script lang="ts">
  /**
   * Six-box single-digit verification-code input (re #21, re #93, re #133).
   *
   * Shared by the suite and admin SPAs via @herold/design-system.
   *
  ...
  // Reset path (re #93, re #133): when value is cleared to '' from the
  // outside (e.g. parent clears code after a wrong-code 401 response),
  // focus the first box so the user can retype without touching the mouse.
  // Uses a plain variable (not $state) so only `value` is tracked as a
  // reactive dependency; no loop risk from the write inside the effect.
  let prevValueLen = 0;
  $effect(() => {
